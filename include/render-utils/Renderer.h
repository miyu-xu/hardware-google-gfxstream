...
public:
    ...
    // Testing only API.
    // Rewire all exisint render threads with Framebuffer after a snapshot load
    // in end2end tests.
    // Real snapshots should not use this because it kills and recreates all
    // render threads.
    //virtual bool rewire_for_testing() = 0;
