...
RenderControlDevice* rcCreateDevice() {
    return ToHandle(new RenderControlDeviceImpl());
}

extern "C" __attribute__((visibility("default")))
void rcDestroyDevice(RenderControlDevice* pDevice) {
    if (pDevice != nullptr) {
...
extern "C" __attribute__((visibility("default")))
int rcCompose(RenderControlDevice* deviceHandle,
              const RenderControlComposition* pComposition,
              uint32_t compositionLayerCount,
              const RenderControlCompositionLayer* pCompositionLayers) {
    const size_t bytesNeeded = sizeof(ComposeDevice_v2) + (sizeof(ComposeLayer) * compositionLayerCount);
