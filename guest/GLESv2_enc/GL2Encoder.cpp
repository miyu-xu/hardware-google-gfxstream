...
struct GfxstreamProgramBinaryInfoV1 {
    struct ShaderInfo {
        GLenum type;
        std::string source;
        std::vector<std::string> externalSamplerNames;
    };
    std::vector<ShaderInfo> originalShaders;

    GLenum hostProgramBinaryFormat;
    std::vector<uint8_t> hostProgramBinary;
};
std::vector<uint8_t> Serialize(const GfxstreamProgramBinaryInfoV1& info) {
    std::vector<uint8_t> ret;

    return ret;
}
std::optional<GfxstreamProgramBinaryInfoV1> Deserialize(const void* bytes, GLsizei length) {
    GfxstreamProgramBinaryInfoV1 info = {};
...
#define GL_PROGRAM_BINARY_FORMAT_GFXSTREAM_V1 0x00V1
