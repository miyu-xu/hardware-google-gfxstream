...
void count_VkGraphicsPipelineCreateInfo(uint32_t featureBits, VkStructureType rootType,
    ...
    *count += 8;
    ...
    // WARNING PTR CHECK
    if (VULKAN_STREAM_FEATURE_IGNORED_HANDLES_BIT) {
        *count += 8;
    }
    if ((!(VULKAN_STREAM_FEATURE_IGNORED_HANDLES_BIT) || toCount->pVertexInputState)) {
    ...
    if ((!(VULKAN_STREAM_FEATURE_IGNORED_HANDLES_BIT) || toCount->pInputAssemblyState)) {
    ...
    *count += 8;
    ...
    if ((!(VULKAN_STREAM_FEATURE_IGNORED_HANDLES_BIT) || toCount->pRasterizationState)) {
