...
VkResult createSync(uint64_t syncId, int64_t& osHandle) {
    struct VirtGpuExecBuffer exec = {};
    struct gfxstreamCreateSync createSync = {};
    VirtGpuDevice* instance = VirtGpuDevice::getInstance();

    createSync.hdr.opCode = GFXSTREAM_CREATE_SYNC;
    createSync.syncId = syncId;

    exec.command = static_cast<void*>(&createSync);
    exec.command_size = sizeof(createSync);
...
VkResult ResourceTracker::on_vkGetSemaphoreFdKHR(void* context, VkResult, VkDevice device,
    ...
    if (getSyncFd) {
        if (mCaps.params[kParamFencePassing] && mCaps.vulkanCapset.externalSync) {
            ...
            result = createSync(syncId, osHandle);
            if (result != VK_SUCCESS) {
