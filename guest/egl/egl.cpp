...
{
    ENCODER_DEBUG_LOG("eglClientWaitSyncKHR(dpy:%p, eglsync:%p, attribute:%d, value:%p)", dpy,
...
EGLint eglWaitSyncKHR(EGLDisplay dpy, EGLSyncKHR eglsync, EGLint flags) {
    ENCODER_DEBUG_LOG("eglDupNativeFenceFDANDROID(dpy:%p, eglsync:%p, flags:%d)", dpy, eglsync,
                      flags);
