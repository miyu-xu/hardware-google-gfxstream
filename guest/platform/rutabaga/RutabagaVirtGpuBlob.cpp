...
RutabagaVirtGpuResource::RutabagaVirtGpuResource(
        ...
        std::shared_ptr<RutabagaVirtGpuDevice> device)
    : mResourceId(resourceId),
      mResourceType(resourceType),
      mDevice(device) {}

RutabagaVirtGpuResource::~RutabagaVirtGpuResource() {
    EmulatedVirtioGpu::Get().UnrefResource(mResourceId);
}
...
int RutabagaVirtGpuResource::transferFromHost(uint32_t offset, uint32_t size) {
    ...
    return EmulatedVirtioGpu::Get().TransferFromHost(mDevice->GetContextId(), mResourceId, offset, size);
}
...
int RutabagaVirtGpuResource::transferToHost(uint32_t offset, uint32_t size) {
    ...
    return EmulatedVirtioGpu::Get().TransferToHost(mDevice->GetContextId(), mResourceId, offset, size);
}
