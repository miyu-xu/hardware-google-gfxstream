...
class RutabagaAHardwareBuffer {
   public:
    RutabagaAHardwareBuffer(uint32_t width, uint32_t height, VirtGpuBlobPtr resource);

    ~RutabagaAHardwareBuffer();
...
class RutabagaGralloc : public Gralloc {
   public:
    RutabagaGralloc();
   ...
   private:
    ...
    // TODO: remove. createColorBuffer() current skips the guest handle.
    std::vector<std::unique_ptr<RutabagaAHardwareBuffer>> mOwned;
};
