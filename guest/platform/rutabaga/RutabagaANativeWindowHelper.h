...
class RutabagaANativeWindow {
  public:
    RutabagaANativeWindow(uint32_t width,
                          uint32_t height,
                          uint32_t format,
  ...
  private:
    ...
    uint32_t mHeight;
    uint32_t mFormat;
    std::vector<std::unique_ptr<RutabagaAHardwareBuffer>> mBuffers;

    struct QueuedAHB {
        RutabagaAHardwareBuffer* ahb;
        int fence = -1;
    };
    std::deque<QueuedAHB> mBufferQueue;
};

class RutabagaANativeWindowHelper : public ANativeWindowHelper {
  public:
    bool Init();
    ...
    EGLNativeWindowType createNativeWindowForTesting(Gralloc* gralloc,
                                                     uint32_t width,
