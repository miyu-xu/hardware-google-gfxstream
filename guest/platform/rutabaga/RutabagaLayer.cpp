...
void EmulatedVirtioGpu::EmulatedVirtioGpuImpl::UnrefResource(uint32_t resourceId) {
    ...
    VirtioGpuTaskUnrefResource task{
        .resourceId = resourceId,
    };
    EnqueueVirtioGpuTask(kInvalidContextId, std::move(task));
}
...
void EmulatedVirtioGpu::EmulatedVirtioGpuImpl::DoTask(VirtioGpuTaskCreateContext task) {
    ALOGV("Performing task to create context-id:%" PRIu32 " context-init:%" PRIu32 " context-name:%s",
    ...
    ALOGV("Performing task to create context-id:%" PRIu32 " context-init:%" PRIu32 " context-name:%s - done",
          task.contextId, task.contextInit, task.contextName.c_str());
}
    ...
    std::visit(
        [this](auto&& work){
            ...
            if constexpr (std::is_same_v<T, VirtioGpuTaskCreateBlob>) {
                ...
                DoTask(std::move(work));
            }   if constexpr (std::is_same_v<T, VirtioGpuTaskTransferToHost>) {
                DoTask(std::move(work));
