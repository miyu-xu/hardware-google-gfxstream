...
class EmulatedVirtioGpu::EmulatedVirtioGpuImpl {
  public:
    ...
    void UnrefResource(uint32_t resourceId);
...
VirtGpuCaps EmulatedVirtioGpu::EmulatedVirtioGpuImpl::GetCaps() {
    VirtGpuCaps caps = {
        .params = {
            [kParam3D] = 1,
            [kParamCapsetFix] = 1,
...
uint32_t EmulatedVirtioGpu::EmulatedVirtioGpuImpl::CreateContext() {
    const uint32_t contextId = mNextContextId++;
    const std::string contextName = "EmulatedVirtioGpu Context " + std::to_string(contextId);
    if (stream_renderer_context_create(contextId, contextName.size(), contextName.data(), 0) != 0) {
        ALOGE("Failed to CreateContext().");
        return -1;
    }
    return contextId;
}
...
void EmulatedVirtioGpu::EmulatedVirtioGpuImpl::DestroyContext(uint32_t contextId) {
    stream_renderer_context_destroy(contextId);
}
...
uint8_t* EmulatedVirtioGpu::EmulatedVirtioGpuImpl::Map(uint32_t resourceId) {
    ...
    if (resource->type == EmulatedResourceType::kBlob) {
        if (!resource->mappedHostBytes.valid()) {
            ALOGE("Failed to Map() resource %" PRIu32 ": attempting to map blob without mappable flag?", resourceId);
            return nullptr;
        }
...
void EmulatedVirtioGpu::EmulatedVirtioGpuImpl::UnrefResource(uint32_t resourceId) {
...
void EmulatedVirtioGpu::EmulatedVirtioGpuImpl::DoTask(VirtioGpuTaskCreateBlob task) {
    ...
    int ret = stream_renderer_create_blob(task.contextId, task.resourceId, &task.params, nullptr, 0, nullptr);
    if (ret) {
...
bool EmulatedVirtioGpu::Init(bool withGl, bool withVk) {
    return mImpl->Init(withGl, withVk, this);
}

uint32_t EmulatedVirtioGpu::CreateContext() {
...
VirtGpuCaps EmulatedVirtioGpu::GetCaps() {
    return mImpl->GetCaps();
}
...
int EmulatedVirtioGpu::TransferFromHost(uint32_t contextId, uint32_t resourceId, uint32_t offset, uint32_t size) {
    return mImpl->TransferFromHost(contextId, resourceId, offset, size);
}

int EmulatedVirtioGpu::TransferToHost(uint32_t contextId, uint32_t resourceId, uint32_t offset, uint32_t size) {
    return mImpl->TransferToHost(contextId, resourceId, offset, size);
}

std::optional<uint32_t> EmulatedVirtioGpu::CreateBlob(uint32_t contextId, const struct VirtGpuCreateBlob& params) {
    return mImpl->CreateBlob(contextId, params);
}

std::optional<uint32_t> EmulatedVirtioGpu::CreatePipeBlob(uint32_t contextId, uint32_t size) {
    return mImpl->CreatePipeBlob(contextId, size);
}

void EmulatedVirtioGpu::UnrefResource(uint32_t resourceId) {
    mImpl->UnrefResource(resourceId);
}
