...
RutabagaVirtGpuDevice::RutabagaVirtGpuDevice(std::shared_ptr<EmulatedVirtioGpu> emulation, uint32_t contextId, VirtGpuCapset capset)
    : VirtGpuDevice(capset), mEmulation(emulation), mContextId(contextId), mCapset(capset) {}

RutabagaVirtGpuDevice::~RutabagaVirtGpuDevice() {
    mEmulation->DestroyContext(mContextId);
}
...
VirtGpuBlobPtr RutabagaVirtGpuDevice::createVirglBlob(uint32_t width, uint32_t height,
    ...
    const auto resourceIdOpt =
        mEmulation->CreateVirglBlob(mContextId, width, height, virglFormat);
    if (!resourceIdOpt) {
        return nullptr;
    }
