...
RutabagaAHardwareBuffer::RutabagaAHardwareBuffer(uint32_t width, uint32_t height,
...
uint32_t RutabagaAHardwareBuffer::getResourceId() const { return mResource->getResourceHandle(); }

uint32_t RutabagaAHardwareBuffer::getWidth() const { return mWidth; }

uint32_t RutabagaAHardwareBuffer::getHeight() const { return mHeight; }

int RutabagaAHardwareBuffer::getAndroidFormat() const {
    return /*AHARDWAREBUFFER_FORMAT_R8G8B8A8_UNORM=*/1;
}

uint32_t RutabagaAHardwareBuffer::getDrmFormat() const { return DRM_FORMAT_ABGR8888; }
...
void RutabagaAHardwareBuffer::acquire() { ++mRefCount; }

void RutabagaAHardwareBuffer::release() {
    --mRefCount;
    if (mRefCount == 0) {
        delete this;
    }
...
AHardwareBuffer* RutabagaGralloc::allocate(uint32_t width, uint32_t height, uint32_t format) {
    ...
    return reinterpret_cast<AHardwareBuffer*>(
        new RutabagaAHardwareBuffer(width, height, std::move(resource)));
}

void RutabagaGralloc::acquire(AHardwareBuffer* ahb) {
    auto* rahb = reinterpret_cast<RutabagaAHardwareBuffer*>(ahb);
    rahb->acquire();
}
...
const native_handle_t* RutabagaGralloc::getNativeHandle(const AHardwareBuffer* ahb) {
...
int RutabagaGralloc::getId(const AHardwareBuffer* ahb, uint64_t* id) {
    const auto* rahb = reinterpret_cast<const RutabagaAHardwareBuffer*>(ahb);
    *id = rahb->getResourceId();
    return 0;
}

bool RutabagaGralloc::Init() {
    mEmulation = EmulatedVirtioGpu::Get();
    return mEmulation != nullptr;
}

Gralloc* createPlatformGralloc(int /*deviceFd*/) {
    auto* gralloc = new RutabagaGralloc();
    if (!gralloc->Init()) {
        ALOGE("Failed to initialize RutabagaGralloc: Failed to get emulation layer.");
        return nullptr;
    }
    return gralloc;
}
