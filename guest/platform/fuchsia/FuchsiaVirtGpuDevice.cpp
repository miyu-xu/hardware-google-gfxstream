...
VirtGpuBlobPtr FuchsiaVirtGpuDevice::createBlob(const struct VirtGpuCreateBlob& blobCreate) {
    ...
    ALOGI("*** createBlob got resource_id %u", response->resource_id);

    return std::make_shared<FuchsiaVirtGpuBlob>(this, response->resource_id,
                                                response->resource_id, blobCreate.size);
}
