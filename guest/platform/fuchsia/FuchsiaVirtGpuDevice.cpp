...
FuchsiaVirtGpuDevice::FuchsiaVirtGpuDevice(enum VirtGpuCapset capset) : VirtGpuDevice(capset) {
    ...
    if (capset == kCapsetGfxStreamVulkan) {
        ...
        if (wire_result.ok() && wire_result->value()->is_buffer_result()) {
            ...
            ALOGE("Query(%lu) failed: status %d, expected buffer result", query_id,
                  wire_result.status());
        }
...
VirtGpuBlobPtr FuchsiaVirtGpuDevice::createBlob(const struct VirtGpuCreateBlob& blobCreate) {
    ...
    builder.size(blobCreate.size)
        .blob_mem(blobCreate.blobMem)
        .id(blobCreate.blobId)
        .flags(blobCreate.flags);
    ...
    ALOGI("*** createBlob got bo_handle %u resource_handle %u", response->bo_handle,
          response->resource_handle);

    return std::make_shared<FuchsiaVirtGpuBlob>(this, response->bo_handle,
                                                response->resource_handle, blobCreate.size);
}
...
int FuchsiaVirtGpuDevice::execBuffer(struct VirtGpuExecBuffer& execbuffer, VirtGpuBlobPtr blob) {
    ALOGI("*** execBuffer size %u ring_idx %u flags 0x%x handle %u", execbuffer.command_size,
          execbuffer.ring_idx, execbuffer.flags, execbuffer.handle);
