...
std::unique_ptr<HostConnection> HostConnection::connect(enum VirtGpuCapset capset,
    ...
    switch (connType) {
        ...
        case HOST_CONNECTION_VIRTIO_GPU_PIPE: {
            auto stream = new VirtioGpuPipeStream(STREAM_BUFFER_SIZE, -1);
            if (!stream) {
...
#if defined(ANDROID)
    descriptor = (con->m_rendernodeFd >= 0) ? con->m_rendernodeFd : descriptor;
    ALOGE("--- descriptor is %d ---", descriptor);
    con->m_grallocHelper.reset(gfxstream::createPlatformGralloc(descriptor));
