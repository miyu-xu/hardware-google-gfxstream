...
static VkResult vk_command_buffer_createOp(struct vk_command_pool*, VkCommandBufferLevel, struct vk_command_buffer**);
...
VkResult gfxstream_vk_AllocateCommandBuffers(VkDevice device,
    ...
    for (uint32_t i = 0; i < pAllocateInfo->commandBufferCount; i++) {
        result = vk_command_buffer_createOp(&gfxstream_commandPool->vk, VK_COMMAND_BUFFER_LEVEL_PRIMARY,
