...
#include "FullscreenColor.h"

namespace gfxstream {
namespace {

#include "FullscreenColor.vert.inl"
#include "FullscreenColor.frag.inl"

}  // namespace

/*static*/
Result<std::unique_ptr<FullscreenColor>> FullscreenColor::Create() {
    std::unique_ptr<FullscreenColor> sample(new FullscreenColor());
...
Result<Ok> FullscreenColor::StartUp() {
    ...
    mPipelineLayout = VK_EXPECT_RV(mDevice->createPipelineLayoutUnique(pipelineLayoutCreateInfo));
    ...
    mVertShaderModule = VK_EXPECT_RV(mDevice->createShaderModuleUnique(vertShaderCreateInfo));
    ...
    mFragShaderModule = VK_EXPECT_RV(mDevice->createShaderModuleUnique(fragShaderCreateInfo));
...
Result<Ok> FullscreenColor::CreateSwapchainDependents(const SwapchainInfo& swapchainInfo) {
    ...
    mRenderpass = VK_EXPECT_RV(mDevice->createRenderPassUnique(renderpassCreateInfo));
    ...
    mPipeline = VK_EXPECT_RV(mDevice->createGraphicsPipelineUnique({}, pipelineCreateInfo));
...
Result<Ok> FullscreenColor::RecordFrame(const FrameInfo& frame) {
    ...
    const SwapchainImageObjects& swapchainObjects = mSwapchainImageObjects[frame.swapchainImageIndex];
