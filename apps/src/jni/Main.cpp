...
#include "fullscreencolor/FullscreenColor.h"
...
void Main(struct android_app* app) {
    AppState state;
    state.sample = VK_ASSERT(FullscreenColor::Create());
    ALOGD("Initialized FullscreenColor.");
    ...
    while (true) {
        ...
        while ((ident = ALooper_pollOnce(state.drawing ? 0 : -1, nullptr, nullptr, (void**)&source)) > ALOOPER_POLL_TIMEOUT) {
            if (source != nullptr) {
