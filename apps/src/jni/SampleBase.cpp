...
Result<uint32_t> GetMemoryType(const vkhpp::PhysicalDevice& physical_device,
    ...
    return gfxstream::unexpected("Failed to find memory type matching " + vkhpp::to_string(memoryProperties));
}
...
Result<Ok> SampleBase::StartUpBase(
    ...
    const vkhpp::ApplicationInfo applicationInfo = {
        .pApplicationName = "Gfxstream Sample App",
        .applicationVersion = 1,
        .pEngineName = "Gfxstream Sample App",
        .engineVersion = 1,
    ...
    if (kEnableValidationLayers) {
        ...
        debugMessenger = VK_EXPECT_RV(mInstance->createDebugUtilsMessengerEXTUnique(debugCreateInfo));
    }

    const auto physicalDevices = VK_EXPECT_RV(mInstance->enumeratePhysicalDevices());
    mPhysicalDevice = std::move(physicalDevices[0]);

    std::unordered_set<std::string> availableDeviceExtensions;
    {
        const auto exts = VK_EXPECT_RV(mPhysicalDevice.enumerateDeviceExtensionProperties());
    ...
    for (const auto& e : requestedDeviceExtensions) {
        if (e == std::string(VK_KHR_SAMPLER_YCBCR_CONVERSION_EXTENSION_NAME)) {
            ...
            if (sampler_features.samplerYcbcrConversion == VK_FALSE) {
                return gfxstream::unexpected("Physical device doesn't support samplerYcbcrConversion");
            }
            ycbcr_conversion_needed = true;
        } else {
            if (availableDeviceExtensions.find(e) == availableDeviceExtensions.end()) {
                return gfxstream::unexpected("Physical device doesn't support extension " + std::string(e));
            }
    ...
    for (uint32_t i = 0; i < kMaxFramesInFlight; i++) {
        ...
        auto readyForRender = VK_EXPECT_RV(mDevice->createSemaphoreUnique(semaphoreCreateInfo));
        auto readyForPresent = VK_EXPECT_RV(mDevice->createSemaphoreUnique(semaphoreCreateInfo));
        auto commandPool = VK_EXPECT_RV(mDevice->createCommandPoolUnique(commandPoolCreateInfo));
        const vkhpp::CommandBufferAllocateInfo commandBufferAllocateInfo = {
            .commandPool = *commandPool,
            .level = vkhpp::CommandBufferLevel::ePrimary,
            .commandBufferCount = 1,
        };
        auto commandBuffers = VK_EXPECT_RV(mDevice->allocateCommandBuffersUnique(commandBufferAllocateInfo));
...
Result<SampleBase::BufferWithMemory> SampleBase::CreateBuffer(
    ...
    const auto bufferMemoryType =
        VK_EXPECT(GetMemoryType(mPhysicalDevice,
    ...
    auto bufferMemory = VK_EXPECT_RV(mDevice->allocateMemoryUnique(bufferMemoryAllocateInfo));
...
Result<SampleBase::BufferWithMemory> SampleBase::CreateBufferWithData(
    ...
    void* mapped = VK_EXPECT_RV(mDevice->mapMemory(*mStagingBuffer.bufferMemory, 0, kStagingBufferSize));
...
Result<std::vector<uint8_t>> SampleBase::DownloadImage(
    ...
    auto* mapped = reinterpret_cast<uint8_t*>(
        VK_EXPECT_RV(mDevice->mapMemory(*mStagingBuffer.bufferMemory, 0, kStagingBufferSize)));
...
Result<Ok> SampleBase::LoadYuvImage(
        const vkhpp::UniqueImage& image,
        uint32_t width,
        uint32_t height,
        ...
        vkhpp::ImageLayout currentLayout,
        vkhpp::ImageLayout returnedLayout) {
    auto* mapped = reinterpret_cast<uint8_t*>(VK_TRY_RV(mDevice->mapMemory(*mStagingBuffer.bufferMemory, 0, kStagingBufferSize)));
    ...
    return DoCommandsImmediate([&](vkhpp::UniqueCommandBuffer& cmd) {
        ...
        cmd->copyBufferToImage(*mStagingBuffer.buffer,
                               *image,
...
Result<SampleBase::FramebufferWithAttachments>
SampleBase::CreateFramebuffer(
        uint32_t width,
        uint32_t height,
...
Result<Ok> SampleBase::CreateSurface() {
    if (mWindow == nullptr) {
        return gfxstream::unexpected("Failed to create VkSurface: no window!");
    }

    const vkhpp::AndroidSurfaceCreateInfoKHR surfaceCreateInfo = {
        .window = mWindow,
    };
    mSurface = VK_EXPECT_RV(mInstance->createAndroidSurfaceKHR(surfaceCreateInfo));
...
Result<Ok> SampleBase::CreateSwapchain() {
    if (!mSurface) {
        return gfxstream::unexpected("Failed to CreateSwapchain(): missing VkSurface?");
    }

    const auto capabilities = VK_EXPECT_RV(mPhysicalDevice.getSurfaceCapabilitiesKHR(*mSurface));
    const vkhpp::Extent2D swapchainExtent = capabilities.currentExtent;

    const auto formats = VK_EXPECT_RV(mPhysicalDevice.getSurfaceFormatsKHR(*mSurface));
    ...
    for (const auto& format : formats) {
        ...
        ALOGI(" - format:%s colorspace:%s", formatStr.c_str(), colorspaceStr.c_str());
    }
    ...
    const auto modes = VK_EXPECT_RV(mPhysicalDevice.getSurfacePresentModesKHR(*mSurface));
    ...
    auto swapchain = VK_EXPECT_RV(mDevice->createSwapchainKHRUnique(swapchainCreateInfo));

    auto swapchainImages = VK_EXPECT_RV(mDevice->getSwapchainImagesKHR(*swapchain));
...
Result<Ok> SampleBase::Render() {
    ...
    VK_EXPECT_RESULT(mDevice->waitForFences({*perFrame.readyFence}, VK_TRUE, UINT64_MAX));
    ...
    vkhpp::Result result = mDevice->acquireNextImageKHR(swapchain, UINT64_MAX, *perFrame.readyForRender, {}, &swapchainImageIndex);
    if (result == vkhpp::Result::eErrorOutOfDateKHR) {
        return RecreateSwapchain();
    } else if (result != vkhpp::Result::eSuccess && result != vkhpp::Result::eSuboptimalKHR) {
        return gfxstream::unexpected("Failed to acquire next image: " + vkhpp::to_string(result));
    }
    ...
    const vkhpp::SubmitInfo submitInfo = {
        ...
        .signalSemaphoreCount = static_cast<uint32_t>(renderSignalSemaphores.size()),
        .pSignalSemaphores = renderSignalSemaphores.data(),
    };
    mQueue.submit(submitInfo, *perFrame.readyFence);

    const std::vector<vkhpp::Semaphore> presentReadySemaphores = { *perFrame.readyForPresent };
    const vkhpp::PresentInfoKHR presentInfo = {
        .waitSemaphoreCount = static_cast<uint32_t>(presentReadySemaphores.size()),
    ...
    if (result == vkhpp::Result::eErrorOutOfDateKHR || result == vkhpp::Result::eSuboptimalKHR) {
        VK_EXPECT(RecreateSwapchain());
    } else if (result != vkhpp::Result::eSuccess) {
        return gfxstream::unexpected("Failed to present image: " + vkhpp::to_string(result));
    }
