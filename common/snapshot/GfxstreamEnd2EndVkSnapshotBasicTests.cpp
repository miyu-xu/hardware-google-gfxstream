...
protected:
	void SetUp() override {
		...
		printf("set kFeature_VulkanSnapshots to true\n");
...
TEST_P(GfxstreamEnd2EndVkSnapshotBasicTest, BasicSaveLoad) {
    ...
    std::unique_ptr<android::base::StdioStream> stream(new android::base::StdioStream(fopen(snapshotFileName.c_str(), "wb"), android::base::StdioStream::kOwner));
    android::snapshot::ITextureSaverPtr textureSaver(new android::snapshot::TextureSaver(
    	android::base::StdioStream(fopen(textureFileName.c_str(), "wb"), android::base::StdioStream::kOwner)));
    stream_renderer_snapshot_presave_pause();
    stream_renderer_snapshot_save(stream.get(), &textureSaver);
    stream.reset();
    textureSaver.reset();
    stream.reset(new android::base::StdioStream(fopen(snapshotFileName.c_str(), "rb"), android::base::StdioStream::kOwner));
    android::snapshot::ITextureLoaderPtr textureLoader(new android::snapshot::TextureLoader(
    	android::base::StdioStream(fopen(textureFileName.c_str(), "rb"), android::base::StdioStream::kOwner)));
