...
TEST_P(GfxstreamEnd2EndVkTest, GetPhysicalDeviceImageFormatProperties2KHR) {
    ...
    const auto properties = VK_ASSERT_VKHPP_RV(physicalDevice.getImageFormatProperties2(imageFormatInfo));
...
TEST_P(GfxstreamEnd2EndVkTest, DescriptorSetAllocFree) {
    ...
    auto descriptorPool = VK_ASSERT_VKHPP_RV(device->createDescriptorPoolUnique(descriptorPoolCreateInfo));
    ...
    auto descriptorSetLayout = VK_ASSERT_VKHPP_RV(device->createDescriptorSetLayoutUnique(descriptorSetLayoutInfo));

    const std::vector<vkhpp::DescriptorSetLayout> descriptorSetLayouts(kNumSets, *descriptorSetLayout);
    const vkhpp::DescriptorSetAllocateInfo descriptorSetAllocateInfo = {
        .descriptorPool = *descriptorPool,
        .descriptorSetCount = kNumSets,
        .pSetLayouts = descriptorSetLayouts.data(),
    };
    auto descriptorSets = VK_ASSERT_VKHPP_RV(device->allocateDescriptorSetsUnique(descriptorSetAllocateInfo));

    std::vector<vkhpp::DescriptorSet> descriptorSetHandles;
    for (auto& ds : descriptorSets) {
        descriptorSetHandles.push_back(*ds);
    }

    EXPECT_THAT(device->freeDescriptorSets(*descriptorPool, kNumSets, descriptorSetHandles.data()), IsVkSuccess());
    // The double free should also work
    EXPECT_THAT(device->freeDescriptorSets(*descriptorPool, kNumSets, descriptorSetHandles.data()), IsVkSuccess());

    // Alloc/free again should also work
    descriptorSets = VK_ASSERT_VKHPP_RV(device->allocateDescriptorSetsUnique(descriptorSetAllocateInfo));
    EXPECT_THAT(device->freeDescriptorSets(*descriptorPool, kNumSets, descriptorSetHandles.data()), IsVkSuccess());
}
