...
#else
    #error "Unimplemented memfd_create() in CreateSharedMemory()"
#endif

    if (fd == -1) {
        return gfxstream::unexpected("Failed to create shared memory: " + std::string(strerror(errno)));
    }
...
gfxstream::expected<Ok, std::string> CheckImportingSharedMemory(vkhpp::PhysicalDevice& physicalDevice) {
    ...
    void* mappedShm = mmap(nullptr, kSize,  PROT_WRITE | PROT_READ, MAP_SHARED, shm.Get(), 0);
    if (mappedShm == MAP_FAILED) {
        return gfxstream::unexpected("Failed to mmap shared memory: " + std::string(strerror(errno)));
    }
