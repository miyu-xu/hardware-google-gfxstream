...
GlExpected<ScopedGlShader> ScopedGlShader::MakeShader(
        GlDispatch& dispatch,
        GLenum type,
...
GlExpected<ScopedGlProgram> ScopedGlProgram::MakeProgram(
    ...
    auto fragShader = GL_EXPECT(ScopedGlShader::MakeShader(dispatch, GL_FRAGMENT_SHADER, fragSource));
...
GlExpected<ScopedGlProgram> ScopedGlProgram::MakeProgram(
    ...
    GLuint program = dispatch.glCreateProgram();
    dispatch.glProgramBinary(program,
                             programBinaryFormat,
...
GlExpected<ScopedGlShader> GfxstreamEnd2EndTest::SetUpShader(
        GLenum type,
        const std::string& source) {
    if (!mGl) {
