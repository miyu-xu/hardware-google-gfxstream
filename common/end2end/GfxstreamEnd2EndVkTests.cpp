...
class GfxstreamEnd2EndVkTest : public GfxstreamEnd2EndTest {
   protected:
    ...
    gfxstream::expected<YuvImageWithMemory, vkhpp::Result> ImportYuvAhb(
        ...
        if (mGralloc->getFormat(ahb) != GFXSTREAM_AHB_FORMAT_YV12) {
            return gfxstream::unexpected(vkhpp::Result::eErrorUnknown);
        }
    ...
    gfxstream::expected<vkhpp::UniquePipelineLayout, vkhpp::Result> CreatePipelineLayout(
        TypicalVkTestEnvironment& vk,
        const std::vector<DescriptorSetBundle*>& descriptorSetLayouts) {
        std::vector<vkhpp::DescriptorSetLayout> descriptorSetLayoutHandles;
        for (const auto* descriptorSetLayout : descriptorSetLayouts) {
            descriptorSetLayoutHandles.push_back(*descriptorSetLayout->layout);
        }
        ...
        return VK_EXPECT_RV(vk.device->createPipelineLayoutUnique(pipelineLayoutCreateInfo));
    }

    struct PipelineParams {
        std::vector<uint32_t> vert;
        std::vector<uint32_t> frag;
        vkhpp::PipelineLayout layout;
...
// jasonjason
TEST_P(GfxstreamEnd2EndVkTest, AhbYv12Render) {
    const uint32_t width = 32;
    const uint32_t height = 32;
    auto ahb = VK_ASSERT(
        ScopedAHardwareBuffer::Allocate(*mGralloc, width, height, GFXSTREAM_AHB_FORMAT_YV12));

    auto vk = VK_ASSERT(SetUpTypicalVkTestEnvironment());
    auto& [instance, physicalDevice, device, queue, queueFamilyIndex] = vk;

    auto ahbImage = VK_ASSERT(ImportYuvAhb(vk, ahb, vkhpp::ImageUsageFlagBits::eSampled,
                                           vkhpp::ImageLayout::eShaderReadOnlyOptimal));

    auto framebuffer = VK_ASSERT(CreateFramebuffer(
    ...
    auto pipelineLayout = VK_ASSERT(CreatePipelineLayout(vk, {&descriptorSet0}));
    auto pipeline = VK_ASSERT(CreatePipeline(vk, {
    ...
    VK_ASSERT(DoCommandsImmediate(vk, [&](vkhpp::UniqueCommandBuffer& cmd) {
    ...
    auto actualImage =
        VK_ASSERT(DownloadImage(vk, width, height, framebuffer.colorAttachment->image,
