...
TEST_P(GfxstreamEnd2EndVkTest, UseDoubleQueuesAndSynchronizeCorrectly) {
    ...
    if (graphicsQueueCount < 2) {
        ALOGW("Device does not support at least 2 graphic_bit queues. Skipping test...");
        return;
    }
    ...
    if (!sourceBufferWithMemoryResult.ok()) {
        ALOGW("Couldn't create hostVisible,hostCoherent,TransferSrc buffer. Skipping test...");
        return;
    }
    ...
    if (!destBufferWithMemoryResult.ok()) {
        ALOGW("Couldn't create hostVisible, hostCoherent, TransferDst buffer. Skipping test...");
        return;
    }
    ...
    commandBufferWriteInto->updateBuffer(sourceBufferWithMemory.buffer.get(), 0, dataToWrite.size(), dataToWrite.data());
    ...
    commandBufferCopy->copyBuffer(sourceBufferWithMemory.buffer.get(), destBufferWithMemory.buffer.get(), copyRegions);
    ...
    queue2.waitIdle();
    void* mappedMemory = GFXSTREAM_ASSERT_VKHPP_RV(device->mapMemory(*destBufferWithMemory.bufferMemory, 0, kBufferSize));
