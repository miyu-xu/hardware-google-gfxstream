...
class GfxstreamEnd2EndVkTest : public GfxstreamEnd2EndTest {
   protected:
    ...
    Result<Ok> FillAhb(ScopedAHardwareBuffer& ahb, PixelR8G8B8A8 color) {
        ...
        if (ahbFormat == GFXSTREAM_AHB_FORMAT_B8G8R8A8_UNORM) {
        ...
        } else if (ahbFormat == GFXSTREAM_AHB_FORMAT_YV12) {
            uint8_t y;
            uint8_t u;
            uint8_t v;
            ConvertRGBA8888PixelToYUV(color.r, color.g, color.b, &y, &u, &v);

            const Gralloc::LockedPlane& yPlane = planes[0];
            const Gralloc::LockedPlane& uPlane = planes[2];
            const Gralloc::LockedPlane& vPlane = planes[1];
            for (uint32_t y = 0; y < ahbHeight; y++) {
                for (uint32_t x = 0; x < ahbWidth; x++) {
                    uint8_t* dstY =
            ...
            return gfxstream::unexpected("Unhandled AHB format");
        }
    ...
    void DoFillAndRenderFromAhb(uint32_t ahbFormat) {
        ...
        constexpr const int kMaxReportedFailures = 10;
        for (uint32_t y = 0; y < height; y++) {
            for (uint32_t x = 0; x < width; x++) {
                const auto actual = PixelR8G8B8A8(x, y, resultImage.pixels[(y * height) + x]);
                if (actual != goldenPixel) {
                    if (reportedFailures < kMaxReportedFailures) {
                        reportedFailures++;
