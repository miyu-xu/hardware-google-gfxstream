...
class ScopedGlType {
  public:
    ...
    ScopedGlType(GlDispatch& glDispatch,
                 GlDispatchGenFunc glGenFunc,
                 GlDispatchDelFunc glDelFunc)
        : mGlDispatch(&glDispatch),
          mGlGenFunc(glGenFunc),
          mGlDelFunc(glDelFunc) {
...
class ScopedGlBuffer : public ScopedGlType {
  public:
    ScopedGlBuffer(GlDispatch& dispatch) :
        ScopedGlType(dispatch,
                     &GlDispatch::glGenBuffers,
                     &GlDispatch::glDeleteBuffers) {}
};
...
class ScopedGlTexture : public ScopedGlType {
  public:
    ScopedGlTexture(GlDispatch& dispatch) :
        ScopedGlType(dispatch,
                     &GlDispatch::glGenTextures,
                     &GlDispatch::glDeleteTextures) {}
};
...
class ScopedGlShader {
  public:
    ...
    static GlExpected<ScopedGlShader> MakeShader(GlDispatch& dispatch,
                                                 GLenum type,
                                                 const std::string& source);

    ScopedGlShader(ScopedGlShader&& rhs)
        : mGlDispatch(rhs.mGlDispatch),
          mHandle(rhs.mHandle) {
        rhs.mHandle = 0;
    }
  ...
  private:
    ScopedGlShader(GlDispatch& dispatch, GLuint handle)
...
class ScopedGlProgram {
  public:
    ...
    static GlExpected<ScopedGlProgram> MakeProgram(
        GlDispatch& dispatch,
    ...
    ScopedGlProgram(ScopedGlProgram&& rhs)
        : mGlDispatch(rhs.mGlDispatch),
          mHandle(rhs.mHandle) {
        rhs.mHandle = 0;
    }
