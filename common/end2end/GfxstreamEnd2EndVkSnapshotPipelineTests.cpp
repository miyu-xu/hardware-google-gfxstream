...
class GfxstreamEnd2EndVkSnapshotPipelineTest : public GfxstreamEnd2EndTest {
   protected:
    ...
    std::unique_ptr<BufferInfo> createAndPopulateBuffer(vkhpp::PhysicalDevice physicalDevice,
                                                        vkhpp::Device device,
                                                        vkhpp::BufferUsageFlags usage,
...
VkExpected<BufferInfo> GfxstreamEnd2EndVkSnapshotPipelineTest::createAndPopulateBuffer(
    ...
    if (vertexBufferMemoryType == -1) {
      return android::base::unexpected(VK_ERROR_OUT_OF_HOST_MEMORY);
    }
    // Vertex memory
    ...
    auto vertexBufferMemory = VK_EXPECT_RV(device.allocateMemoryUnique(vertexBufferMemoryAllocateInfo));
...
TEST_P(GfxstreamEnd2EndVkSnapshotPipelineTest, CanSnapshotDescriptors) {
    ...
    auto pipelineInfo = createPipeline(device.get());
    auto vertexBufferInfo = createAndPopulateBuffer(physicalDevice, device.get(),
                                                    vkhpp::BufferUsageFlagBits::eVertexBuffer,
                                                    kVertexData, sizeof(kVertexData));
    ...
    std::vector<uint32_t> dst(kFbWidth * kFbHeight);
    utils::readImageData(*colorAttachmentInfo->image, kFbWidth, kFbHeight, vkhpp::ImageLayout::eColorAttachmentOptimal,
        dst.data(),
                  dst.size() * sizeof(uint32_t), testEnvironment);
    for (int i = 0; i < dst.size(); i++) {
