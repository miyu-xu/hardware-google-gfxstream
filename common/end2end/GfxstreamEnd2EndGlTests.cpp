...
struct PixelR8G8B8A8 {
    PixelR8G8B8A8(uint8_t rr, uint8_t gg, uint8_t bb, uint8_t aa)
        : r(rr), g(gg), b(bb), a(aa) {}
    ...
    friend void PrintTo(const PixelR8G8B8A8& pixel, std::ostream* os) {
        *os << pixel.ToString();
    }
};

MATCHER_P4(IsOkWithRGBA, r, g, b, a, std::string(" equals ") + PixelR8G8B8A8(r, g, b, a).ToString()) {
  ...
  protected:
    GlExpected<PixelR8G8B8A8> GetPixelAt(GLint x, GLint y) {
        ...
        if (GLenum error = mGl->glGetError(); error != GL_NO_ERROR) {
            return android::base::unexpected("Failed to glReadPixels() with error " + std::to_string(error));
        }
    ...
    void SetUp() override {
        ...
        SetUpEglContextAndSurface(2, mSurfaceWidth, mSurfaceHeight, &mDisplay, &mContext, &mSurface);
    }
...
TEST_P(GfxstreamEnd2EndGlTest, CreateWindowSurface) {
    ...
    int numConfigs = 0;
    ASSERT_THAT(mGl->eglChooseConfig(mDisplay, configAttributes, nullptr, 1, &numConfigs), IsTrue());
    ASSERT_THAT(numConfigs, Gt(0));

    EGLConfig config = nullptr;
    ASSERT_THAT(mGl->eglChooseConfig(mDisplay, configAttributes, &config, 1, &numConfigs), IsTrue());
...
TEST_P(GfxstreamEnd2EndGlTest, SwitchContext) {
    ASSERT_THAT(mGl->eglMakeCurrent(mDisplay, EGL_NO_SURFACE, EGL_NO_SURFACE, EGL_NO_CONTEXT), IsTrue());
    for (int i = 0; i < 100; i++) {
...
TEST_P(GfxstreamEnd2EndGlTest, ProgramBinary) {
    const uint32_t width = 2;
    const uint32_t height = 2;
    AHardwareBuffer* ahb = nullptr;
    ASSERT_THAT(mGralloc->allocate(width, height, DRM_FORMAT_ABGR8888, -1, &ahb), Eq(0));

    ALOGE("jasonjason finished allocate.");
    {
        uint8_t* mapped = nullptr;

        ALOGE("jasonjason locking.");
        ASSERT_THAT(mGralloc->lock(ahb, &mapped), Eq(0));
        ...
        ASSERT_THAT(mGralloc->unlock(ahb), Eq(0));
        ALOGE("jasonjason finished unlock.");
    }
...

    {
        ...
        for (uint32_t h = 0; h < height; h++) {
            for (uint32_t w = 0; w < width; w++) {
                ASSERT_THAT(mapped[pos++], Eq(0));
                ASSERT_THAT(mapped[pos++], Eq(0));
                ASSERT_THAT(mapped[pos++], Eq(128));
                ASSERT_THAT(mapped[pos++], Eq(255));
            }
        }
    ...
    mGl->glTexImage2D(GL_TEXTURE_2D, 0, GL_RGBA, kTextureWidth, kTextureHeight, 0, GL_RGBA, GL_UNSIGNED_BYTE, kTextureData);
    ...
    std::vector<uint8_t> programBinaryData;
    {
        ...
        mGl->glGetProgramBinary(program,
                                programBinaryLength,
                                &readProgramBinaryLength,
                                &programBinaryFormat,
                                programBinaryData.data());
        ASSERT_THAT(mGl->glGetError(), Eq(GL_NO_ERROR));
    ...
    mGl->glTexImage2D(GL_TEXTURE_2D, 0, GL_RGBA, kFramebufferWidth, kFramebufferHeight, 0, GL_RGBA, GL_UNSIGNED_BYTE, nullptr);
    mGl->glFramebufferTexture2D(GL_FRAMEBUFFER, GL_COLOR_ATTACHMENT0, GL_TEXTURE_2D, framebufferTexture, 0);
    ASSERT_THAT(mGl->glGetError(), Eq(GL_NO_ERROR));
