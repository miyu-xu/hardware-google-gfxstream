...
MATCHER_P(IsOkWith, expected, std::string(" equals ") + expected.ToString()) {
    ...
    if (actual.ok() &&
        actual.value().r == expected.r &&
        actual.value().g == expected.g &&
        actual.value().b == expected.b &&
        actual.value().a == expected.a) {
        return true;
    }
...
TEST_P(GfxstreamEnd2EndGlTest, TextureUploadAndReadbackWithAhb) {
    ...
    // Update AHB with `uploadPixel` via texture upload:
    {
        ...
        mGl->glTexSubImage2D(GL_TEXTURE_2D, 0, 0, 0, width, height, GL_RGBA, GL_UNSIGNED_BYTE, uploadPixels.data());
        ASSERT_THAT(mGl->glGetError(), Eq(GL_NO_ERROR));
...
TEST_P(GfxstreamEnd2EndGlTest, TextureUploadAndBlitWithAhb) {
    const uint32_t width = 2;
    const uint32_t height = 2;
    ...
    mGl->glFramebufferTexture2D(GL_FRAMEBUFFER, GL_COLOR_ATTACHMENT0, GL_TEXTURE_2D, blitFramebufferTexture, 0);
    ...
    // Blit from AHB to an additional framebuffer and readback:
    {
        ...
        ScopedGlProgram program = GL_ASSERT(SetUpProgram(blitTextureVertSource, blitTextureFragSource));
        ...
        ScopedGlBuffer buffer(*mGl);
        mGl->glBindBuffer(GL_ARRAY_BUFFER, buffer);
        mGl->glBufferData(GL_ARRAY_BUFFER, sizeof(VertexAttributes) * fullscreenTriVerts.size(), fullscreenTriVerts.data(), GL_STATIC_DRAW);
        ...
        mGl->glBindBuffer(GL_ARRAY_BUFFER, buffer);
        mGl->glEnableVertexAttribArray(0);
        mGl->glVertexAttribPointer(0, 2, GL_FLOAT, GL_FALSE, sizeof(VertexAttributes), (GLvoid*)offsetof(VertexAttributes, pos));
        mGl->glEnableVertexAttribArray(1);
        mGl->glVertexAttribPointer(1, 2, GL_FLOAT, GL_FALSE, sizeof(VertexAttributes), (GLvoid*)offsetof(VertexAttributes, tex));
        ASSERT_THAT(mGl->glGetError(), Eq(GL_NO_ERROR));
...
TEST_P(GfxstreamEnd2EndGlTest, TextureUploadAndBlitExternalOesWithAhb) {
    const uint32_t width = 2;
    const uint32_t height = 2;

    ALOGE("jasonjason readback setup:");
    ...
    ScopedGlFramebuffer readbackFramebuffer(*mGl);
    mGl->glBindFramebuffer(GL_FRAMEBUFFER, readbackFramebuffer);
    mGl->glFramebufferTexture2D(GL_FRAMEBUFFER, GL_COLOR_ATTACHMENT0, GL_TEXTURE_2D, readbackTexture, 0);
    ASSERT_THAT(mGl->glCheckFramebufferStatus(GL_FRAMEBUFFER), Eq(GL_FRAMEBUFFER_COMPLETE));
    ASSERT_THAT(mGl->glGetError(), Eq(GL_NO_ERROR));
...
TEST_P(GfxstreamEnd2EndGlTest, MultiThreadedTextureUploadAndReadbackWithAhb) {
    const uint32_t width = 2;
    const uint32_t height = 2;
    ...
    std::thread readbackThread([&]() {
        ...
        SetUpEglContextAndSurface(2, 32, 32, &mDisplay, &readbackThreadContext, &readbackThreadSurface);
        {
            // Create a framebuffer for blitting the AHB into and reading back the blitted results from:
            ScopedGlTexture readbackTexture(*mGl);
            mGl->glBindTexture(GL_TEXTURE_2D, readbackTexture);
            mGl->glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_MAG_FILTER, GL_LINEAR);
            mGl->glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_MIN_FILTER, GL_LINEAR);
            ...
            // Create program using program binaries:
            GLenum programBinaryFormat = GL_NONE;
            std::vector<uint8_t> programBinaryData;
            {
                const std::string vertSource = R"(\
                ...
                GLint readProgramBinaryLength = 0;
                mGl->glGetProgramBinary(program, programBinaryLength, &readProgramBinaryLength, &programBinaryFormat, programBinaryData.data());
                ASSERT_THAT(mGl->glGetError(), Eq(GL_NO_ERROR));
                ASSERT_THAT(programBinaryLength, Eq(readProgramBinaryLength));
            }
            ScopedGlProgram program = GL_ASSERT(SetUpProgram(programBinaryFormat, programBinaryData));
            ...
            {
                EGLImageKHR readbackAhbImage = mGl->eglCreateImageKHR(mDisplay, EGL_NO_CONTEXT, EGL_NATIVE_BUFFER_ANDROID, ahb, ahbImageAttribs);
                ...
                mGl->glReadPixels(0, 0, width, height, GL_RGBA, GL_UNSIGNED_BYTE, readbackBytes.data());
                ...
                for (uint32_t y = 0; y < height; y++) {
                    for (uint32_t x = 0; x < width; x++) {
                        ...
                        readbackPixel.x = x;
                        readbackPixel.y = y;
                        readbackPixel.r = *readbackBytesPtr; ++readbackBytesPtr;
                        readbackPixel.g = *readbackBytesPtr; ++readbackBytesPtr;
                        readbackPixel.b = *readbackBytesPtr; ++readbackBytesPtr;
                        readbackPixel.a = *readbackBytesPtr; ++readbackBytesPtr;
    ...
    // "MainThread" waits on upload fence:
    {
        ...
        ASSERT_THAT(uploadCompleteFence, Not(Eq(EGL_NO_SYNC)));
        mGl->eglClientWaitSyncKHR(mDisplay, uploadCompleteFence, /*flags=*/0, /*timeout=2 seconds*/2 * 1000 * 1000 * 1000);
        ASSERT_THAT(mGl->eglGetError(), Eq(EGL_SUCCESS));
