...
TestingVirtGpuResource::TestingVirtGpuResource(
    ...
    : mResourceId(resourceId),
        ...
        mResourceMappedHostBytes(std::move(resourceMappedBytes)) {}
...
VirtGpuBlobMappingPtr TestingVirtGpuResource::createMapping(void) {
    ...
    if (mResourceType == ResourceType::kBlob) {
        mappedMemory = mResourceMappedHostBytes->value_or(nullptr);
...
VirtGpuBlobPtr TestingVirtGpuDevice::createBlob(const struct VirtGpuCreateBlob& blobCreate) {
    ...
    EnqueueVirtioGpuTask(std::move(createTask));

    auto mappedHostBytes = std::make_unique<std::optional<uint8_t*>>();

    VirtioGpuTaskMap mapTask{
        .resourceId = resourceId,
        .resourceMappedHostBytes = mappedHostBytes.get(),
    };
    EnqueueVirtioGpuTask(std::move(mapTask));

    // Note: virtio gpu kernel doesn't seem to attach fence for blobs.
    std::shared_future<void> completedFuture = std::async(std::launch::deferred, [] {}).share();
    completedFuture.wait();

    return TestingVirtGpuResource::createBlob(resourceId, shared_from_this(), completedFuture, std::move(mappedHostBytes));
}
...
void TestingVirtGpuDevice::DoTask(VirtioGpuTaskMap task) {
    ...
    task.resourceMappedHostBytes->emplace(reinterpret_cast<uint8_t*>(mapped));
...
GfxstreamEnd2EndTest::SetUpTypicalVkTestEnvironment(uint32_t apiVersion) {
    ...
    auto instance = VK_EXPECT_VKHPP_RV(vkhpp::createInstanceUnique(instanceCreateInfo));

    VULKAN_HPP_DEFAULT_DISPATCHER.init(*instance);

    auto physicalDevices = VK_EXPECT_VKHPP_RV(instance->enumeratePhysicalDevices());
    ...
    {
        const auto exts = VK_EXPECT_VKHPP_RV(physicalDevice.enumerateDeviceExtensionProperties());
    ...
    auto device = VK_EXPECT_VKHPP_RV(physicalDevice.createDeviceUnique(deviceCreateInfo));
