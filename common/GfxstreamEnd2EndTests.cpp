...
class TestingVirtGpuDevice : public std::enable_shared_from_this<TestingVirtGpuDevice>, public VirtGpuDevice {
    ...
    int WaitOnEmulatedFence(int fenceAsFileDescriptor, int timeoutMilliseconds) {
...

        {
            ...
            if (fenceIt == mVirtioGpuFences.end()) {
                ALOGV("Fence:%d already signaled", (int)fenceId);
                return 0;
            }
        ...
        if (status == std::future_status::ready) {
            ...
            ALOGV("Timed out waiting for fence:%d", (int)fenceId);
            return -1;
        }
    }
  ...
  private:
    ...
    void DoTask(VirtioGpuTaskCreateResource task) {
        ALOGV("Performing task to create resource resource-id:%d", task.resourceId);
        ...
        ALOGV("Performing task to create resource resource-id:%d - done", task.resourceId);
...
TEST_P(GfxstreamEnd2EndVkTest, HostMemory) {
    ...
    void* bytes = memory.mapMemory(0, VK_WHOLE_SIZE, vkhpp::MemoryMapFlags{});
