...
std::unique_ptr<TestingAHardwareBuffer> TestingVirtGpuGralloc::allocate(
        ...
        uint32_t height,
        uint32_t format) {
    ALOGE("Allocating AHB w:%" PRIu32 " h:%" PRIu32 " f:%" PRIu32, width, height, format);
...
void GfxstreamEnd2EndTest::SetUp() {
    ...
    ASSERT_THAT(setenv("GFXSTREAM_EMULATED_VIRTIO_GPU_WITH_GL", params.with_gl ? "Y" : "N", /*overwrite=*/1), Eq(0));
    ASSERT_THAT(setenv("GFXSTREAM_EMULATED_VIRTIO_GPU_WITH_VK", params.with_vk ? "Y" : "N", /*overwrite=*/1), Eq(0));
