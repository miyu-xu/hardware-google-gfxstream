...
int VirtioGpuFrontend::platformResourceInfo(int res_handle, int* width, int* height,
                                            int* internal_format) {
    bool success = false;
    auto it = mResources.find(res_handle);
    if (it == mResources.end()) return -EINVAL;
...
int VirtioGpuFrontend::exportBlob(uint32_t resourceId, struct stream_renderer_handle* handle) {
    ...
    if (!entry.descriptorInfo) {
        stream_renderer_error("Failed to export blob for resource %d: no descriptor.", resourceId);
        return -EINVAL;
    }
    ...
    if (rawDescriptorOpt) {
        ...
        stream_renderer_error("Failed to export blob for resource %d: descriptor release failed.",
                              resourceId);
        return -EINVAL;
    }
