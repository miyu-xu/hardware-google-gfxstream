...
int VirtioGpuFrontend::getResourceInfo(uint32_t resourceId, struct stream_renderer_resource_info* info) {
...
int VirtioGpuFrontend::createBlob(uint32_t contextId,
                                  uint32_t resourceId,
                                  const struct stream_renderer_create_blob* createBlobArgs,
                                  const struct stream_renderer_handle* handle) {
    auto contextIt = mContexts.find(contextId);
    if (contextIt == mContexts.end()) {
        stream_renderer_error("failed to create blob resource %u: context %u missing.", resourceId, contextId);
        return -EINVAL;
    }
    auto& context = contextId->second;
    ...
    if (blobIt != context.blobMap.end()) {
        ...
        blobMap.erase(blobIt);
    }

    auto resourceOpt = VirtioGpuResource::Create(contextId, resourceId, createArgs, createBlobArgs, handle);
    if (!resourceOpt) {
