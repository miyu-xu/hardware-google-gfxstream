...
int VirtioGpuFrontend::importResource(uint32_t res_handle, const struct stream_renderer_handle* import_handle,
    ...
    if (!resourceOpt) {
        stream_renderer_error("Failed to create resource %u, with import_handle/import_data", res_handle);
        return -EINVAL;
    }
