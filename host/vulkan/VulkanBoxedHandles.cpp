...
BoxedHandleManager::BoxedHandleManager(const gfxstream::host::FeatureSet& featureSet)
...
void delete_VkType(BoxedHandleManager* pBoxedHandleManager, VkObjectT boxed) {
    ...
    if (info->readStream) {
        pBoxedHandleManager->getReadStreamRegistry().push(info->readStream);
...
VkQueue unbox_VkQueueImpl(BoxedHandleManager* pBoxedHandleManager, VkQueue boxed) {
    ...
    if (pBoxedHandleManager->getFeatures().VulkanVirtualQueue.enabled) {
...
VulkanMemReadingStream* get_read_stream_VkType(BoxedHandleManager* pBoxedHandleManager,
    ...
    if (info->readStream == nullptr) {
        info->readStream =
            pBoxedHandleManager->getReadStreamRegistry().pop(pBoxedHandleManager->getFeatures());
    }
