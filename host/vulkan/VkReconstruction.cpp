...
std::vector<VkReconstruction::HandleWithState> typeTagSortedHandles(const std::vector<VkReconstruction::HandleWithState>& handles) {
    ...
    std::sort(res.begin(), res.end(), [](const VkReconstruction::HandleWithState& lhs, const VkReconstruction::HandleWithState& rhs) {
    ...
    mHandleReconstructions.forEachLiveComponent_const(
        [&totalParents, &next](bool live, uint64_t componentHandle, uint64_t entityHandle,
            ...
            for (int state = BEGIN; state < HANDLE_STATE_COUNT; state ++) {
                ...
                HandleWithState handleWithState = {
                    entityHandle, static_cast<HandleState>(state)
                };
                totalParents[handleWithState] =
                        parents.size();
                if (parents.empty()) {
                    next.push_back(handleWithState);
                }
                printf("live handle 0x%lx\n", entityHandle);
            }
        });
...
void VkReconstruction::addHandleDependency(const uint64_t* handles, uint32_t count,
                                           uint64_t parentHandle, HandleState childState, HandleState parentState) {
    if (!handles) return;
    ...
    for (uint32_t i = 0; i < count; ++i) {
        ...
        parentItemState.childHandles.insert(
            { handles[i], static_cast<HandleState>(childState) });
