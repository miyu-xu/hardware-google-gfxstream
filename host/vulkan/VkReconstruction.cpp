...
uint64_t VkReconstruction::getLastModifyApiOpHandle(uint64_t commandBuffer) {
    ...
    fprintf(stderr, "%s %s %d failed: the api 0x%llx name %s does no create any handles\n",
            __FILE__, __func__, __LINE__, apiHandle, itemApi ? api_opcode_to_string(itemApi->opCode) : "unknown");
    return 0;
}
