...
class SharedLibraries {
   public:
    ...
    bool addFirstAvailableLibrary(const std::vector<std::string>& possiblePaths) {
        ERR("jasonjason %s:%d:%s", __FILE__, __LINE__, __PRETTY_FUNCTION__);
        for (const std::string& possiblePath : possiblePaths) {
            ERR("jasonjason %s:%d:%s possible:%s", __FILE__, __LINE__, __PRETTY_FUNCTION__, possiblePath.c_str());
...
#error "Unhandled platform in VulkanDispatchImpl."
    ...
    std::vector<std::string> getPossibleLoaderPaths() {
        ...
        if (mForTesting || explicitIcd == "mock") {
            ERR("jasonjason %s:%d:%s", __FILE__, __LINE__, __PRETTY_FUNCTION__);
            const std::vector<std::string> possibleDirectories = {
    ...
    std::vector<std::string> getPossibleMoltenVkPaths() {
        ...
        const std::string explicitPath =
            android::base::getEnvironmentVariable("ANDROID_EMU_VK_LOADER_PATH");
        if (!explicitPath.empty()) {
            ERR("jasonjason %s:%d:%s explicit:%s", __FILE__, __LINE__, __PRETTY_FUNCTION__, explicitPath.c_str());
            return {
                explicitPath,
            };
        }
        ...
        ERR("jasonjason %s:%d:%s customIcd:%s", __FILE__, __LINE__, __PRETTY_FUNCTION__, customIcd.c_str());
