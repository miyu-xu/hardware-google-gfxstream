...
std::optional<SwapchainCreateInfoWrapper> SwapChainStateVk::createSwapChainCi(
    ...
    if (iSurfaceFormat == formats.end()) {
        ERR("Fail to create swapchain: the format(%#" PRIx64
    ...
    if (!presentModes.count(VK_PRESENT_MODE_FIFO_KHR)) {
        ERR("Fail to create swapchain: FIFO present mode not supported.");
        return std::nullopt;
    }
    ...
    if (!maybeExtent.has_value()) {
        ERR("Fail to create swapchain: extent(%" PRIu64 "x%" PRIu64
                                ") not supported.",
    ...
    if (queueFamilyIndices.empty()) {
        ERR("Fail to create swapchain: no Vulkan queue family specified.");
        return std::nullopt;
    }
