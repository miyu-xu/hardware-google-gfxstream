...
bool ColorBufferVk::importExtMemoryHandle(void *extMemHandle, uint32_t extMemStreamHandleType) {
    return importExtMemoryHandleToVkColorBuffer(mHandle, extMemStreamHandleType, (ExternalHandleType)extMemHandle);
}
...
std::optional<BlobDescriptorInfo> ColorBufferVk::exportBlob() {
    ...
    if (info) {
        return BlobDescriptorInfo{
            .descriptorInfo = {
                .descriptor = ManagedDescriptor(info->handleInfo.handle),
