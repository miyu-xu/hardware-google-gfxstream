...
#define _RUN_AND_CHECK(command) { if (command) GFXSTREAM_ABORT(emugl::FatalError(emugl::ABORT_REASON_OTHER)) << "Vulkan snapshot save failed at " << __func__ << " (" << __FILE__ << ":" << __LINE__ << ")"; }
...
void saveImageContent(android::base::Stream* stream, StateBlock* stateBlock, VkImage image, const ImageInfo* imageInfo) {
    ...
    _RUN_AND_CHECK(dispatch->vkAllocateCommandBuffers(stateBlock->device, &allocInfo, &commandBuffer) != VK_SUCCESS);
    ...
    _RUN_AND_CHECK(dispatch->vkCreateBuffer(stateBlock->device, &bufferCreateInfo, nullptr, &readbackBuffer));

    VkMemoryRequirements readbackBufferMemoryRequirements{};
    dispatch->vkGetBufferMemoryRequirements(
    ...
    _RUN_AND_CHECK(dispatch->vkAllocateMemory(stateBlock->device, &readbackBufferMemoryAllocateInfo, nullptr, &readbackMemory));
    _RUN_AND_CHECK(dispatch->vkBindBufferMemory(stateBlock->device, readbackBuffer, readbackMemory, 0));

    void* mapped = nullptr;
    _RUN_AND_CHECK(dispatch->vkMapMemory(stateBlock->device, readbackMemory, 0, VK_WHOLE_SIZE, VkMemoryMapFlags{}, &mapped));
...
void loadImageContent(android::base::Stream* stream, StateBlock* stateBlock, VkImage image, const ImageInfo* imageInfo) {
    VkEmulation* vkEmulation = getGlobalVkEmulation();
    VulkanDispatch* dispatch = vkEmulation->dvk;
    const VkImageCreateInfo& imageCreateInfo = imageInfo->imageCreateInfoShallow;
    VkCommandBufferAllocateInfo allocInfo {
    ...
    _RUN_AND_CHECK(dispatch->vkCreateBuffer(stateBlock->device, &bufferCreateInfo, nullptr, &stagingBuffer));

    VkMemoryRequirements stagingBufferMemoryRequirements{};
    dispatch->vkGetBufferMemoryRequirements(
        stateBlock->device, stagingBuffer, &stagingBufferMemoryRequirements);
    ...
    _RUN_AND_CHECK(dispatch->vkAllocateMemory(stateBlock->device, &stagingBufferMemoryAllocateInfo, nullptr, &stagingMemory));
    _RUN_AND_CHECK(dispatch->vkBindBufferMemory(stateBlock->device, stagingBuffer, stagingMemory, 0));

    void* mapped = nullptr;
    _RUN_AND_CHECK(dispatch->vkMapMemory(stateBlock->device, stagingMemory, 0, VK_WHOLE_SIZE, VkMemoryMapFlags{}, &mapped));
