...
bool allocExternalMemory(VulkanDispatch* vk, VkEmulation::ExternalMemoryInfo* info,
    ...
    if (exportRes != VK_SUCCESS || VK_EXT_MEMORY_HANDLE_INVALID == info->externalHandle) {
        WARN("allocExternalMemory: Failed to get external memory, native handle: %s",
...
bool initializeVkColorBufferLocked(
    ...
    if (!isFormatVulkanCompatible(infoPtr->internalFormat)) {
        WARN("Failed to create Vk ColorBuffer: format:%d not compatible.", infoPtr->internalFormat);
        return false;
    }
    ...
    if (createRes != VK_SUCCESS) {
        WARN("Failed to create Vulkan image for ColorBuffer %d, error: %s", colorBufferHandle,
