...
bool initializeVkColorBufferLocked(
    uint32_t colorBufferHandle, VkImageLayout initLayout = VK_IMAGE_LAYOUT_UNDEFINED, VK_EXT_MEMORY_HANDLE extMemHandle = VK_EXT_MEMORY_HANDLE_INVALID) {
    ...
    printf("created image %p for color buffer\n", infoPtr->image);
...
bool importExtMemoryHandleToVkColorBuffer(uint32_t colorBufferHandle, uint32_t type,
    ...
    return initializeVkColorBufferLocked(colorBufferHandle, VK_IMAGE_LAYOUT_UNDEFINED, extMemHandle);
}
