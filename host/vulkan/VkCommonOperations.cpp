...
std::optional<ExternalHandleInfo> VkEmulation::exportMemoryHandle(VkDevice device,
                                                                  VkDeviceMemory memory) {
    ExternalHandleInfo ret;
    ...
    ret.handle = (ExternalHandleType)fd;
    ...
    if (mDeviceInfo.getMemoryHandleFunc(mDevice, &memoryGetHandleInfo, &ret.handle) != VK_SUCCESS) {
        return std::nullopt;
    }
#else
...
// TODO(aruby@qnx.com): Remove once dmabuf extension support has been flushed out on QNX
    ...
    {
        android::base::AutoLock lock(*emulation->mQueueLock);
...
#ifdef __APPLE__
static MTLResource_id VkEmulation::getMtlResourceFromVkDeviceMemory(VulkanDispatch* vk,
                                                                    VkDeviceMemory memory) {
    ...
    vk->vkGetMemoryMetalHandleEXT(device, &getMetalHandleInfo, &outputHandle);
...
bool VkEmulation::teardownVkColorBufferLocked(uint32_t colorBufferHandle) {
    ...
    if (infoPtr->initialized) {
        auto& info = *infoPtr;
        {
            android::base::AutoLock lock(*mQueueLock);
...
void VkEmulation::setColorBufferLatestUse(uint32_t colorBufferHandle, DeviceOpWaitable waitable,
...
int VkEmulation::waitSyncVkColorBuffer(uint32_t colorBufferHandle) {
    std::lock_guard<std::mutex> lock(mMutex);
    auto infoPtr = android::base::find(mColorBuffers, colorBufferHandle);
    if (!infoPtr) {
        ERR("Invalid ColorBuffer handle %d.", static_cast<int>(colorBufferHandle));
