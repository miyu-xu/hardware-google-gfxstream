...
void initVkEmulationFeatures(std::unique_ptr<VkEmulationFeatures> features) {
    ...
    if (sVkEmulation->representativeColorBufferMemoryTypeInfo) {
        VK_COMMON_VERBOSE("Representative ColorBuffer memory type using host memory type index %d "
...
findRepresentativeColorBufferMemoryTypeIndexLocked() {
    ...
    if (!getColorBufferAllocationInfoLocked(kArbitraryHandle, nullptr, &hostMemoryTypeIndex, nullptr,
                                            nullptr)) {
    ...
    EmulatedPhysicalDeviceMemoryProperties helper(sVkEmulation->deviceInfo.memProps, hostMemoryTypeIndex, sVkEmulation->features);
