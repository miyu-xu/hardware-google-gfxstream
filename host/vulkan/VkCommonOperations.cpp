...
bool initializeVkColorBufferLocked(
    ...
    if ((VK_EXT_MEMORY_HANDLE_INVALID != extMemHandle) &&
        ...
        VK_COMMON_ERROR(
            "Failed to initialize Vk ColorBuffer -- extMemHandle (0x%p) provided, but device does "
    ...
    if (VK_EXT_MEMORY_HANDLE_INVALID != extMemHandle) {
        if (!updateExternalMemoryInfo(extMemHandle, &infoPtr->memReqs, &infoPtr->memory)) {
            VK_COMMON_ERROR(
                "Failed to update external memory info for ColorBuffer: %d (extMemHandle: 0x%p)\n",
                colorBufferHandle, (void*)extMemHandle);
            return false;
        }
