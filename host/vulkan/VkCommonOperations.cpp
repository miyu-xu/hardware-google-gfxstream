...
// Otherwise try to find the best device with discrete GPU, external memory support and
// high vulkan API level. Scoring of the devices is done by some implicit choices
// based on known driver quality, stability and performance issues of current GPUs.
...
int getSelectedGpuIndex(const std::vector<VkEmulation::DeviceSupportInfo>& deviceInfos) {
    ...
    auto getDeviceScore = [](const VkEmulation::DeviceSupportInfo& deviceInfo) {
        ...
        // External memory support helps with performance
        if (deviceInfo.supportsExternalMemoryImport) {
            deviceScore += 500;
        }
        if (deviceInfo.supportsExternalMemoryExport) {
            deviceScore += 500;
        }
