...
static bool updateExternalMemoryInfo(std::optional<ExternalHandleInfo> extMemHandleInfo,
    ...
    if (!((1 << pInfo->typeIndex) & screenBufferProps.memoryTypeBits)) {
        ERR("QNX Screen buffer can not be imported to memory with typeIndex=%d, screenBufferProps.memoryTypeBits=0x%x", pInfo->typeIndex, screenBufferProps.memoryTypeBits);
        return false;
    }
