...
bool initializeVkColorBufferLocked(
    ...
    if (addConversion) {
        ...
        createRes = vk->vkCreateSamplerYcbcrConversion(sVkEmulation->device, &ycbcrCreateInfo,nullptr, &infoPtr->ycbcrConversion);
        if (createRes != VK_SUCCESS) {
            VERBOSE("Failed to create Vulkan ycbcrConversion for ColorBuffer %d with format %s [%d], Error: %s",
                    colorBufferHandle, string_VkFormat(imageVkFormat), imageVkFormat, string_VkResult(createRes));
            return false;
        }
