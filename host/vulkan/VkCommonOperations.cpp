...
std::optional<ExternalHandleType> dupExternalMemory(ExternalHandleType h,
...
bool importExternalMemory(VulkanDispatch* vk, VkDevice targetDevice,
    ...
    if (!importInfoPtr) {
        auto dupHandle =
            dupExternalMemory(info->handleInfo.handle, info->handleInfo.streamHandleType);
        if (!dupHandle) {
...
std::optional<VkColorBufferMemoryExport> exportColorBufferMemory(uint32_t colorBufferHandle) {
    ...
    auto dupHandle =
        dupExternalMemory(info->memory.handleInfo.handle, info->memory.handleInfo.streamHandleType);
    if (!dupHandle) {
