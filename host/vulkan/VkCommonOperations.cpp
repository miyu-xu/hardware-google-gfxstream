...
static uint32_t getValidMemoryTypeIndex(uint32_t indices, VkMemoryPropertyFlags memoryProperty = 0) {
    ...
    if (!found) {
        GFXSTREAM_ABORT(FatalError(ABORT_REASON_OTHER)) << "Could not find memory index for indices %d, with memory properties: %s", indices,
...
bool readColorBufferToBytesLocked(uint32_t colorBufferHandle, uint32_t x, uint32_t y, uint32_t w,
    ...
    auto originalLayout = colorBufferInfo->currentLayout;
    ...
    if (originalLayout != VK_IMAGE_LAYOUT_UNDEFINED)
    {
        ...
        colorBufferInfo->currentLayout = originalLayout;
    }
