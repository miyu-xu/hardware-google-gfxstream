...
static uint32_t lastGoodTypeIndexWithMemoryProperties(uint32_t indices,
    ...
    if (secondBest == ~0) {
        ERR("Could not find memory index for indices %d, with memory properties: %s", indices,
            string_VkMemoryPropertyFlags(memoryProperty).c_str());
        return 0;
    }
    return secondBest;
}
...
std::optional<VkColorBufferMemoryExport> exportColorBufferMemory(uint32_t colorBufferHandle) {
    ...
    return VkColorBufferMemoryExport{
        .descriptor = std::move(dupExternalMemory(info->memory.externalHandle)),
