   ...
   public:
    ...
    VkResult on_vkAllocateMemory(android::base::BumpPool* pool, VkDevice boxed_device,
        ...
        if (hostVisible && m_emu->features.VulkanAllocateHostMemory.enabled &&
            ...
            VKDGS_LOG("Switching memory import index with pMemoryHostPointerProperties.memoryTypeBits = %d", pMemoryHostPointerProperties.memoryTypeBits);

            if (pMemoryHostPointerProperties.memoryTypeBits == 0) {
                // Memory import operation not supported by the driver.
                return VK_ERROR_INCOMPATIBLE_DRIVER;
            }
            auto assignFirstSupportedMemoryTypeIndex = [&](){
            ...
            if ((localAllocInfo.memoryTypeIndex & pMemoryHostPointerProperties.memoryTypeBits) == 0) {
                // TODO Consider assigning the correct index as early as possible, not right before allocation.
                VKDGS_LOG("Detected memoryTypeIndex violation on requested host memory import. Switching to a supported memory index.");
                assignFirstSupportedMemoryTypeIndex();
                VKDGS_LOG("New localAllocInfo memoryTypeIndex %d", localAllocInfo.memoryTypeIndex);
            }
