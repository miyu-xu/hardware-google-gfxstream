   ...
   public:
    ...
    void save(android::base::Stream* stream) {
        ...
        for (const auto& it: mMemoryInfo) {
            ...
            stream->putBe64(reinterpret_cast<uint64_t>(unboxed_to_boxed_non_dispatchable_VkDeviceMemory(it.first)));
    ...
    void load(android::base::Stream* stream, GfxApiLogger& gfxLogger,
        ...
        for (uint32_t i = 0; i < memoryCount; i++) {
            ...
            if (it == mMemoryInfo.end()) {
                GFXSTREAM_ABORT(FatalError(ABORT_REASON_OTHER)) << "Snapshot load failure: cannot find memory handle for " << boxedMemory;
            }
            VkDeviceSize size = stream->getBe64();
            printf("loaded size %lu current size %lu ptr %p\n", size, it->second.size, it->second.ptr);
            if (size != it->second.size || !it->second.ptr) {
                GFXSTREAM_ABORT(FatalError(ABORT_REASON_OTHER)) << "Snapshot load failure: memory size does not match for " << boxedMemory;
            }
    ...
    VkResult on_vkAllocateMemory(android::base::BumpPool* pool, VkDevice boxed_device,
        ...
        printf("allocating memory size %lu\n", memoryInfo.size);
        ...
        *pMemory = new_boxed_non_dispatchable_VkDeviceMemory(*pMemory);
        printf("allocated memory %p ptr %p\n", *pMemory, memoryInfo.ptr);
