   ...
   public:
        ...
        for (uint32_t i = 0; i < descriptorWriteCount; i++) {
            ...
            for (uint32_t j = 0; j < descriptorWrite.descriptorCount; j++) {
                ...
                if (imgViewInfo->boundColorBuffer && descriptorSetInfo) {
                    printf("descriptorSetInfo->boundColorBuffers.insert ds %p cb %d\n",
                            descriptorWrite.dstSet,
                            imgViewInfo->boundColorBuffer.value());
    ...
    VkResult on_vkAllocateMemory(android::base::BumpPool* pool, VkDevice boxed_device,
        ...
        if (importCbInfoPtr) {
            ...
            printf("vkAllocateMemory bound cb %d\n", memoryInfo.boundColorBuffer.value());
        }
                ...
                for (int i = 0; i < submitCount; i++) {
                    for (int j = 0; j < getCommandBufferCount(pSubmits[i]); j++) {
                        ...
                        for (auto descriptorSet : cmdBufferInfo->allDescriptorSets) {
                            printf("committing with descriptor set %p\n", descriptorSet);
                            auto descriptorSetInfo = android::base::find(mDescriptorSetInfo,
                                    descriptorSet);
                            ...
                            if (descriptorSetInfo->boundColorBuffers.size()) {
                                printf("binding cb in descriptor set\n");
                            }
                        }
