...
#define DEFINE_EXTERNAL_MEMORY_PROPERTIES_TRANSFORM(type)                                 \
   ...
   private:
    ...
    std::vector<const char*> filteredDeviceExtensionNames(VulkanDispatch* vk,
        ...
        std::vector<const char*> kHostAlwaysDeviceExtensions = {
            VK_KHR_EXTERNAL_MEMORY_EXTENSION_NAME,
...
#if defined(__APPLE__)
        if (m_vkEmulation->supportsMoltenVk()) {
            kHostAlwaysDeviceExtensions.push_back(VK_KHR_PORTABILITY_SUBSET_EXTENSION_NAME);
            kHostAlwaysDeviceExtensions.push_back(VK_EXT_METAL_OBJECTS_EXTENSION_NAME);
            kHostAlwaysDeviceExtensions.push_back(VK_EXT_EXTERNAL_MEMORY_METAL_EXTENSION_NAME);
        } else {
            // Non-MoltenVK path, use memory_fd
            kHostAlwaysDeviceExtensions.push_back(VK_KHR_EXTERNAL_MEMORY_FD_EXTENSION_NAME);
        }
#endif
        ...
        if (m_vkEmulation->supportsDmaBuf()) {
            kHostAlwaysDeviceExtensions.push_back(VK_EXT_EXTERNAL_MEMORY_DMA_BUF_EXTENSION_NAME);
        }
#endif

        // Enable all the device extensions that should always be enabled on the host (if available)
        for (auto extName : kHostAlwaysDeviceExtensions) {
