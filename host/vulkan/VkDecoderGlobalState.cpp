...
class VkDecoderGlobalState::Impl {
   public:
    ...
    void save(android::base::Stream* stream) {
        ...
        std::vector<VkDescriptorSet> sortedBoxedDescriptorSets;
        for (const auto& descriptorSetIte : mDescriptorSetInfo) {
            auto boxed = unboxed_to_boxed_non_dispatchable_VkDescriptorSet(descriptorSetIte.first);
            if (boxed) {
                sortedBoxedDescriptorSets.push_back(boxed);
            }
        }
        for (const auto& boxedDescriptorSet : sortedBoxedDescriptorSets) {
            auto unboxedDescriptorSet = unbox_VkDescriptorSet(boxedDescriptorSet);
            const DescriptorSetInfo& descriptorSetInfo = mDescriptorSetInfo[unboxedDescriptorSet];
                ...
                for (int j = 0; j < descriptorSetInfo.allWrites[i].size(); j++) {
                    ...
                    switch (entry.writeType) {
                        case DescriptorSetInfo::DescriptorWriteType::ImageInfo:
                            {
                                auto imageInfo = entry.imageInfo;
                        ...
                        case DescriptorSetInfo::DescriptorWriteType::BufferInfo:
                            {
                                auto bufferInfo = entry.bufferInfo;
                                // Get the unboxed version
                        ...
                        case DescriptorSetInfo::DescriptorWriteType::BufferView:
                            {
                                // Get the unboxed version
                                auto bufferView = unboxed_to_boxed_non_dispatchable_VkBufferView(entry.bufferView);
