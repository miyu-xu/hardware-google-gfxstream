...
class VkDecoderGlobalState::Impl {
   public:
    ...
    void destroyDeviceWithExclusiveInfo(VkDevice device,
        ...
        for (auto fenceInfoIt = fenceInfos.begin(); fenceInfoIt != fenceInfos.end(); ) {
            ...
            if (fenceInfo.device == device) {
                destroyFenceWithExclusiveInfo(device, deviceDispatch, deviceInfo, fence, fenceInfo, nullptr, /*allowExternalFenceRecycling=*/false);
    ...
    void destroyBufferWithExclusiveInfo(VkDevice device, VulkanDispatch* deviceDispatch, VkBuffer buffer,
    ...
    void destroyImageWithExclusiveInfo(VkDevice device, VulkanDispatch* deviceDispatch, VkImage image,
                                       ImageInfo& imageInfo, const VkAllocationCallbacks* pAllocator) {
    ...
    void destroyImageViewLocked(VkDevice device, VulkanDispatch* deviceDispatch,
        ...
        destroyImageViewWithExclusiveInfo(device, deviceDispatch, imageView, imageViewInfo, pAllocator);
    ...
    void destroySamplerWithExclusiveInfo(VkDevice device, VulkanDispatch* deviceDispatch, VkSampler sampler,
    ...
    void destroySemaphoreLocked(VkDevice device, VulkanDispatch* deviceDispatch,
        ...
        destroySemaphoreWithExclusiveInfo(device, deviceDispatch, semaphore, semaphoreInfo, pAllocator);
    ...
    DestroyFenceStatus destroyFenceWithExclusiveInfo(VkDevice device, VulkanDispatch* deviceDispatch,
                                                     DeviceInfo& deviceInfo,
                                                     VkFence fence, FenceInfo& fenceInfo,
                                                     const VkAllocationCallbacks* pAllocator,
    ...
    void destroyFenceLocked(VkDevice device, VulkanDispatch* deviceDispatch, VkFence fence,
        ...
        auto destroyStatus = destroyFenceWithExclusiveInfo(device, deviceDispatch, deviceInfo, fence, fenceInfo, pAllocator, /*allowExternalFenceRecycling=*/true);
    ...
    void destroyDescriptorSetLayoutWithExclusiveInfo(VkDevice device, VulkanDispatch* deviceDispatch,
    ...
    void cleanupDescriptorPoolAllocedSetsLocked(DescriptorPoolInfo& descriptorPoolInfo,
    ...
    void destroyDescriptorPoolWithExclusiveInfo(VkDevice device, VulkanDispatch* deviceDispatch,
                                                VkDescriptorPool descriptorPool,
                                                DescriptorPoolInfo& descriptorPoolInfo,
                                                std::unordered_map<VkDescriptorSet, DescriptorSetInfo>& descriptorSetInfos,
                                                const VkAllocationCallbacks* pAllocator) {
        cleanupDescriptorPoolAllocedSetsLocked(descriptorPoolInfo, descriptorSetInfos, true /* destroy */);
    ...
    void destroyDescriptorPoolLocked(VkDevice device, VulkanDispatch* deviceDispatch,
        ...
        destroyDescriptorPoolWithExclusiveInfo(device, deviceDispatch, descriptorPool, descriptorPoolInfo, mDescriptorSetInfo, pAllocator);
    ...
    void resetDescriptorPoolInfoLocked(VkDescriptorPool descriptorPool) {
        ...
        cleanupDescriptorPoolAllocedSetsLocked(descriptorPoolInfo, mDescriptorSetInfo, /*isDestroy=*/false);
    }
    ...
    void destroyShaderModuleLocked(VkDevice device, VulkanDispatch* deviceDispatch,
        ...
        destroyShaderModuleWithExclusiveInfo(device, deviceDispatch, shaderModule, shaderModuleInfo, pAllocator);
    ...
    void destroyPipelineCacheWithExclusiveInfo(VkDevice device, VulkanDispatch* deviceDispatch,
                                               VkPipelineCache pipelineCache, PipelineCacheInfo& pipelineCacheInfo,
                                               const VkAllocationCallbacks* pAllocator) {
    ...
    void destroyPipelineCacheLocked(VkDevice device, VulkanDispatch* deviceDispatch,
        ...
        destroyPipelineCacheWithExclusiveInfo(device, deviceDispatch, pipelineCache, pipelineCacheInfo, pAllocator);
    ...
    void destroyPipelineLocked(VkDevice device, VulkanDispatch* deviceDispatch, VkPipeline pipeline,
        ...
        destroyPipelineWithExclusiveInfo(device, deviceDispatch, pipeline, pipelineInfo, pAllocator);
    ...
    void destroyCommandPoolWithExclusiveInfo(VkDevice device, VulkanDispatch* deviceDispatch,
                                             VkCommandPool commandPool, CommandPoolInfo& commandPoolInfo,
    ...
    void destroyCommandPoolLocked(VkDevice device, VulkanDispatch* deviceDispatch,
        ...
        auto commandPoolInfoIt = mCmdPoolInfo.find(commandPool);
        if (commandPoolInfoIt == mCmdPoolInfo.end()) return;
        auto& commandPoolInfo = commandPoolInfoIt->second;

        destroyCommandPoolWithExclusiveInfo(device, deviceDispatch, commandPool, commandPoolInfo, mCmdBufferInfo, pAllocator);

        mCmdPoolInfo.erase(commandPoolInfoIt);
    }
    ...
    void destroyCommandBufferWithExclusiveInfos(VkDevice device, VulkanDispatch* deviceDispatch,
                                                VkCommandBuffer commandBuffer, CommandBufferInfo& commandBufferInfo,
    ...
    void destroyCommandBufferLocked(VkDevice device, VulkanDispatch* deviceDispatch,
        ...
        auto commandBufferInfoIt = mCmdBufferInfo.find(commandBuffer);
        if (commandBufferInfoIt == mCmdBufferInfo.end()) return;
        auto& commandBufferInfo = commandBufferInfoIt->second;

        destroyCommandBufferWithExclusiveInfos(device, deviceDispatch, commandBuffer, commandBufferInfo, mCmdPoolInfo);

        mCmdBufferInfo.erase(commandBufferInfoIt);
    }
    ...
    void destroyRenderPassLocked(VkDevice device, VulkanDispatch* deviceDispatch,
        ...
        destroyRenderPassWithExclusiveInfo(device, deviceDispatch, renderPass, renderPassInfo, pAllocator);
    ...
    void destroyFramebufferWithExclusiveInfo(VkDevice device, VulkanDispatch* deviceDispatch,
                                             VkFramebuffer framebuffer, FramebufferInfo& framebufferInfo,
                                             const VkAllocationCallbacks* pAllocator) {
    ...
    void destroyFramebufferLocked(VkDevice device, VulkanDispatch* deviceDispatch,
        ...
        destroyFramebufferWithExclusiveInfo(device, deviceDispatch, framebuffer, framebufferInfo, pAllocator);
...
#include "VkSubDecoder.cpp"
    ...
    AsyncResult registerQsriCallback(VkImage boxed_image, VkQsriTimeline::Callback callback) {
        ...
        if (!anbInfo->vk) {
            ERR("Attempted to register QSRI callback on VkImage:%p with uninitialized ANB info.", image);
            return AsyncResult::FAIL_AND_CALLBACK_NOT_SCHEDULED;
        }
        // Could be null or mismatched image, check later
        if (image != anbInfo->image) {
            ERR("Attempted on register QSRI callback on VkImage:%p with wrong image %p.", image, anbInfo->image);
            return AsyncResult::FAIL_AND_CALLBACK_NOT_SCHEDULED;
        }
...
#define DEFINE_BOXED_NON_DISPATCHABLE_HANDLE_API_IMPL(type)                                       \
   ...
   private:
    ...
    void extractInfosWithDeviceInto(VkDevice device, VkHandleToInfoMap& inputMap, VkHandleToInfoMap& outputMap) {
        for (auto it = inputMap.begin(); it != inputMap.end(); it++) {
            auto& info = it->second;
            if (info.device == device) {
                outputMap.insert(inputMap.extract(it));
            }
        }
    }
    ...
    void extractInstanceAndDependenciesLocked(VkInstance instance, InstanceObjects& objects) {
        ...
        for (auto [device, physicalDevice] : mDeviceToPhysicalDevice) {
            ...
            extractInfosWithDeviceInto(device, mCmdBufferInfo, deviceObjects.commandBuffers);
            extractInfosWithDeviceInto(device, mCmdPoolInfo, deviceObjects.commandPools);
    ...
    void destroyInstanceObjects(InstanceObjects& objects) {
        ...
        for (InstanceObjects::DeviceObjects& deviceObjects : objects.devices) {
            ...
            for (auto& [semaphore, semaphoreInfo] : deviceObjects.semaphores) {
                destroySemaphoreWithExclusiveInfo(device, deviceDispatch, semaphore, semaphoreInfo, nullptr);
            }

            for (auto& [sampler, samplerInfo] : deviceObjects.samplers) {
                destroySamplerWithExclusiveInfo(device, deviceDispatch, sampler, samplerInfo, nullptr);
            }
            ...
            for (auto& [imageView, imageViewInfo] : deviceObjects.imageViews) {
                destroyImageViewWithExclusiveInfo(device, deviceDispatch, imageView, imageViewInfo, nullptr);
            }
            ...
            for (auto& [commandBuffer, commandBufferInfo] : deviceObjects.commandBuffers) {
                destroyCommandBufferWithExclusiveInfos(device, deviceDispatch, commandBuffer, commandBufferInfo, deviceObjects.commandPools);
            }

            for (auto& [commandPool, commandPoolInfo] : deviceObjects.commandPools) {
                destroyCommandPoolWithExclusiveInfo(device, deviceDispatch, commandPool, commandPoolInfo, deviceObjects.commandBuffers, nullptr);
            }

            for (auto& [descriptorPool, descriptorPoolInfo] : deviceObjects.descriptorPools) {
                destroyDescriptorPoolWithExclusiveInfo(device, deviceDispatch, descriptorPool, descriptorPoolInfo, deviceObjects.descriptorSets, nullptr);
            }

            for (auto& [descriptorSetLayout, descriptorSetLayoutInfo] : deviceObjects.descriptorSetLayouts) {
                destroyDescriptorSetLayoutWithExclusiveInfo(device, deviceDispatch, descriptorSetLayout, descriptorSetLayoutInfo, nullptr);
            }

            for (auto& [shaderModule, shaderModuleInfo] : deviceObjects.shaderModules) {
                destroyShaderModuleWithExclusiveInfo(device, deviceDispatch, shaderModule, shaderModuleInfo, nullptr);
            }

            for (auto& [pipeline, pipelineInfo] : deviceObjects.pipelines) {
                destroyPipelineWithExclusiveInfo(device, deviceDispatch, pipeline, pipelineInfo, nullptr);
            }

            for (auto& [pipelineCache, pipelineCacheInfo] : deviceObjects.pipelineCaches) {
                destroyPipelineCacheWithExclusiveInfo(device, deviceDispatch, pipelineCache, pipelineCacheInfo, nullptr);
            }

            for (auto& [framebuffer, framebufferInfo] : deviceObjects.framebuffers) {
                destroyFramebufferWithExclusiveInfo(device, deviceDispatch, framebuffer, framebufferInfo, nullptr);
            }

            for (auto& [renderPass, renderPassInfo] : deviceObjects.renderPasses) {
                destroyRenderPassWithExclusiveInfo(device, deviceDispatch, renderPass, renderPassInfo, nullptr);
            }

            destroyDeviceWithExclusiveInfo(device, deviceObjects.device.mapped(), deviceObjects.fences, deviceObjects.queues, nullptr);
        }
