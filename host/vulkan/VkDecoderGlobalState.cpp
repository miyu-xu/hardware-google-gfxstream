   ...
   public:
    ...
    void on_vkGetPhysicalDeviceMemoryProperties(
        ...
        for (uint32_t i = 0; i < pMemoryProperties->memoryTypeCount; ++i) {
            ...
            if (!m_emu->features.GlDirectMem.enabled && !m_emu->features.VirtioGpuNext.enabled) {
    ...
    bool usingDirectMapping() const {
        return m_emu->features.GlDirectMem.enabled || m_emu->features.VirtioGpuNext.enabled;
    }
...
const gfxstream::host::FeatureSet& VkDecoderGlobalState::getFeatures() const {
    return mImpl->getFeatures();
}
