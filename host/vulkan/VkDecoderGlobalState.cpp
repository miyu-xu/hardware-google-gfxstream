   ...
   public:
...

        {
            ...
            for (uint32_t i = 0; i < fenceCount; i++) {
                ...
                if (fenceInfo->external) {
                    ...
                    fenceInfo->emulatedSignalPromise.reset(new std::promise<void>());
                    fenceInfo->emulatedSignal =
                        fenceInfo->emulatedSignalPromise->get_future().share();
                }
            }
        }
    ...
    VkResult on_vkQueueSubmit(android::base::BumpPool* pool, VkQueue boxed_queue,
        ...
        if (releasedColorBuffers.empty()) {
            ...
            std::thread t([flushColorBuffer = m_emu->callbacks.flushColorBuffer,
                           queueCompletedWaitable = std::move(queueCompletedWaitable),
                           releasedColorBuffers, usedFence, emulatedSignalPromise, vk, device]() {
