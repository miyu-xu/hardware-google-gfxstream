   ...
   public:
    ...
    void load(android::base::Stream* stream, GfxApiLogger& gfxLogger,
        ...
        for (const auto& boxedDescriptorPool : sortedBoxedDescriptorPools) {
            ...
            std::vector<VkWriteDescriptorSet> writeDesciptorSets;
            ...
            for (uint64_t poolId : poolInfo.poolIds) {
                ...
                writeStartingIndices.push_back(writeDesciptorSets.size());
                ...
                for (int write = 0; write < validWriteCount; write++) {
                    ...
                    writeDesciptorSets.push_back(writeDescriptorSet);
                }
            }
            ...
            on_vkQueueCommitDescriptorSetUpdatesGOOGLE(&bumpPool,
                    ...
                    writeDesciptorSets.size(), writeDesciptorSets.data());
            _PR_LINE
        }
        _PR_LINE
    ...
    void on_vkQueueCommitDescriptorSetUpdatesGOOGLE(
        ...
        if (didAlloc) {
            ...
            for (uint32_t i = 0; i < descriptorSetCount; ++i) {
                ...
                printf("i %d pendingDescriptorWriteCount %d writeEndIndex %d\n", i, descriptorSetCount, writeEndIndex);
                _PR_LINE
                for (uint32_t j = writeStartIndex; j < writeEndIndex; ++j) {
                    //printf("i: %d i size %d j: %d j size %d\n",
                    //    i, (int)setsToUpdate.size(), j, (int)writeDescriptorSetsForHostDriver.size());
