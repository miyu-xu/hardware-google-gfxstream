   ...
   public:
            ...
            if (!m_vkEmulation->getColorBufferAllocationInfo(
                ...
                if (m_vkEmulation->supportsMoltenVk()) {
                    if (dedicatedAllocInfoPtr == nullptr || localDedicatedAllocInfo.image == VK_NULL_HANDLE) {
                        ...
                        .pNext = nullptr,
                        .image = m_vkEmulation->getColorBufferVkImage(importCbInfoPtr->colorBuffer),
                    ...
                    MTLResource_id cbExtMemoryHandle =
                        m_vkEmulation->getColorBufferMetalMemoryHandle(importCbInfoPtr->colorBuffer);
    ...
    VkResult vkGetBlobInternal(VkDevice boxed_device, VkDeviceMemory memory, uint64_t hostBlobId) {
        ...
        if (m_vkEmulation->getFeatures().SystemBlob.enabled && info->sharedMemory.has_value()) {
            ...
            auto exportedHandleOpt = m_vkEmulation->exportMemoryHandle(device, memory);
            if (!exportedHandleOpt) {
                return VK_ERROR_OUT_OF_HOST_MEMORY;
            }
            auto& exportedHandle = *exportedHandleOpt;

            ManagedDescriptor managedHandle(static_cast<DescriptorType>(exportedHandle.handle));
            ExternalObjectManager::get()->addBlobDescriptorInfo(
                virtioGpuContextId, hostBlobId, std::move(managedHandle),
                exportedHandle.streamHandleType, info->caching,
    ...
    VkResult on_vkQueueSubmit(android::base::BumpPool* pool, VkSnapshotApiCallInfo*,
...

        {
            ...
            for (const auto& colorBuffer : imageBarrierColorBuffers) {
                m_vkEmulation->setColorBufferLatestUse(colorBuffer, queueCompletedWaitable,
