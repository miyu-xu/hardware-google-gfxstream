...
class VkDecoderGlobalState::Impl {
   public:
    ...
    VkResult exportSemaphore(
        ...
        VkResult result = vk->vkGetSemaphoreWin32HandleKHR(device, &getWin32, outHandle);
        if (result != VK_SUCCESS) {
            return result;
        }
        ...
        VkResult result = vk->vkGetSemaphoreFdKHR(device, &getFd, outHandle);
        if (result != VK_SUCCESS) {
            return result;
        }
#else
        return VK_OUT_OF_HOST_MEMORY;
#endif
