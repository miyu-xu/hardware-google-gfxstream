    ...
    VkResult on_vkQueueSubmit(android::base::BumpPool* pool, VkQueue boxed_queue,
        ...
        if (releasedColorBuffers.empty()) {
            ...
            SyncThread::get()->triggerGeneral(
                [flushColorBuffer = m_emu->callbacks.flushColorBuffer,
                 queueCompletedWaitable = std::move(queueCompletedWaitable), releasedColorBuffers,
                 usedFence, emulatedSignalPromise, vk, device]() {
