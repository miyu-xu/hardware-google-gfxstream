    ...
    VkResult on_vkCreateFence(android::base::BumpPool* pool, VkDevice boxed_device,
        ...
        *pFence = VK_NULL_HANDLE;
...

        {
            ...
            fenceInfo.vk = vk;
            fenceInfo.emulatedSignalPromise.reset(new std::promise<void>());
            fenceInfo.emulatedSignal = fenceInfo.emulatedSignalPromise->get_future().share();
    ...
    VkResult on_vkQueueSubmit(android::base::BumpPool* pool, VkQueue boxed_queue,
        ...
        std::shared_ptr<std::promise<void>> emulatedSignalPromise;
        {
            ...
            if (fence) {
                auto* fenceInfo = android::base::find(mFenceInfo, fence);
                if (fenceInfo) {
                    emulatedSignalPromise = fenceInfo->emulatedSignalPromise;
                }
            }
        }
        ...
        if (releasedColorBuffers.empty()) {
            if (emulatedSignalPromise.get()) {
                emulatedSignalPromise->set_value();
            }
        } else {
            std::thread t(
                [flushColorBuffer = m_emu->callbacks.flushColorBuffer, queueCompletedWaitable = std::move(queueCompletedWaitable), releasedColorBuffers, usedFence, emulatedSignalPromise, vk, device]() {
                    vk->vkWaitForFences(device, 1, &usedFence, true, 10ULL * 1000ULL * 1000ULL * 1000ULL);
...
#include "VkSubDecoder.cpp"
    ...
    VkResult waitForFences(uint32_t fenceCount, const VkFence* pFences, bool waitAll, uint64_t timeout) {
        ...
        for (uint32_t i = 0; i < fenceCount; i++) {
            ...
            std::shared_future<void> emulatedSignal;
            {
                ...
                emulatedSignal = mFenceInfo[fence].emulatedSignal;
            }
            // Current implementation does not respect waitAll here.
            // TODO: fix the timeout calculation.
            if (std::future_status::ready !=
                    emulatedSignal.wait_for(std::chrono::nanoseconds(timeout))) {
                return VK_TIMEOUT;
            }
        }
        // TODO: fix the timeout calculation.
        return vk->vkWaitForFences(device, fenceCount, pFences,
                                   waitAll, timeout);
    }
    ...
    VkResult getFenceStatus(VkFence fence) {
        ...
        VulkanDispatch* vk;
        {
            ...
            if (std::future_status::ready !=
                    mFenceInfo[fence].emulatedSignal.wait_for(std::chrono::nanoseconds(0LL))) {
                return VK_NOT_READY;
            }
