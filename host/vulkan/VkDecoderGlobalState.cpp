   ...
   public:
    ...
    T* get(uint64_t h) {
        ...
        if(!ret) {
            ...
            x++;
            ERR(STRINGIFY(T) " - uh oh!, %d 0x%llx", (int)x, h);
            if (x > 123) {
                x = 0;
                ERR("uh oh! xx, %d", (int)x);
            }
        }
        return ret;
    }
   ...
   public:
    ...
    bool registerRenderPassBeginInfo(VkCommandBuffer commandBuffer,
        ...
        if (!pRenderPassBegin) {
            ...
            onFatalVulkanErrorLocked(VK_NULL_HANDLE);
            return false;
        }
...
#include "VkSubDecoder.cpp"
    ...
    void onFatalVulkanError(VkDevice device) {
        std::lock_guard<std::mutex> lock(mMutex);
        return onFatalVulkanErrorLocked(device);
    }

    void onFatalVulkanErrorLocked(VkDevice device) REQUIRES(mMutex) {
