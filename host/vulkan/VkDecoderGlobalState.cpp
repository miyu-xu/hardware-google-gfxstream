        ...
        if (importCbInfoPtr) {
            ...
            if (!getColorBufferAllocationInfo(importCbInfoPtr->colorBuffer,
                ...
                if (opaqueFd && m_emu->deviceInfo.supportsExternalMemoryImport) {
                    ...
                    ExternalHandleType cbExtMemoryHandle = getColorBufferExtMemoryHandle(
                        importCbInfoPtr->colorBuffer, &cbExtMemStreamHandleType);
                    ...
                    if (!dupHandle) {
                        ERR("Failed to duplicate external memory handle: 0x%x, type: 0x%x, for "
                            "ColorBuffer: %d",
                            cbExtMemoryHandle, cbExtMemStreamHandleType,
                            importCbInfoPtr->colorBuffer);
                        return VK_ERROR_OUT_OF_DEVICE_MEMORY;
                    }
#if defined(_WIN32)
                    ...
                    if (STREAM_MEM_HANDLE_TYPE_SCREEN_BUFFER_QNX == cbExtMemStreamHandleType) {
                        ...
                        ERR("Stream mem handleType: 0x%x not support for ColorBuffer import",
                            cbExtMemStreamHandleType);
                        return VK_ERROR_OUT_OF_DEVICE_MEMORY;
                    }
#else
                    // For handles imported with ImportFdInfo, no need to wrap the descriptor as a
                    // managed handle, as the ownership will get transferred to the Vulkan
                    // implementation. From VkImportMemoryFdInfoKHR spec: Importing memory from a
                    // file descriptor transfers ownership of the file descriptor from the
                    // application to the Vulkan implementation. The application must not perform
                    // any operations on the file descriptor after a successful import. The imported
                    // memory object holds a reference to its payload.
            ...
            if (opaqueFd && m_emu->deviceInfo.supportsExternalMemoryImport) {
                ...
                ExternalHandleType bufferExtMemoryHandle = getBufferExtMemoryHandle(
                    importBufferInfoPtr->buffer, &bufferExtMemStreamHandleType);
                ...
                auto dupHandle =
                    dupExternalMemory(bufferExtMemoryHandle, bufferExtMemStreamHandleType);
                if (!dupHandle) {
                    ERR("Failed to duplicate external memory handle: 0x%x, for Buffer: %d",
                        bufferExtMemoryHandle, importBufferInfoPtr->buffer);
                    return VK_ERROR_OUT_OF_DEVICE_MEMORY;
                }
