...
class VkDecoderGlobalState::Impl {
   public:
    ...
    void save(android::base::Stream* stream) {
        ...
        const bool skipSnapshotData =
            (android::base::getEnvironmentVariable("ANDROID_EMU_VK_SKIP_SNAPSHOT_DATA") == "1");
        if (skipSnapshotData) {
    ...
    void on_vkDestroyPipelineLayout(android::base::BumpPool*, VkSnapshotApiCallInfo*,
        ...
        // vkDestroyPipelineLayout calls can be delayed and dispatcher checks
        // from the autogenerated code won't work. We need to validate the dispatcher
        if (!deviceDispatch) {
...
#include "VkSubDecoder.cpp"
    ...
    VkDescriptorSet getOrAllocateDescriptorSetFromPoolAndIdLocked(
        ...
        auto* poolInfo = android::base::find(mDescriptorPoolInfo, pool);
        if (!poolInfo) {
            on_FatalError(StringFormat("Cannot find pool info %p", pool).c_str());
            return VK_NULL_HANDLE;
        }

        BoxedHandleInfo* setHandleInfo = sBoxedHandleManager.get(poolId);
        if (!setHandleInfo) {
            on_FatalError(StringFormat("Cannot find pool handle for %llu", poolId).c_str());
            return VK_NULL_HANDLE;
        }
        ...
        if (setHandleInfo->underlying) {
            ...
            if (pendingAlloc) {
                ...
                *didAlloc = true;
                ...
                on_FatalError(
                    StringFormat("Descriptor pool %p wanted to get set with id 0x%x", pool, poolId)
                        .c_str());
                return VK_NULL_HANDLE;
            }
        }
    ...
    void on_vkQueueCommitDescriptorSetUpdatesGOOGLE(
        ...
        if (!vk || !queueInfo) {
            on_FatalError(
                StringFormat("Cannot commit descriptor set updates on queue: %p", queue).c_str());
            return;
        }

        on_vkQueueCommitDescriptorSetUpdatesGOOGLELocked(
            pool, snapshotInfo, vk, queueInfo->device, descriptorPoolCount, pDescriptorPools,
    ...
    void on_FatalError(const char* errorMessage) {
        // The recovery mode is opt-in for now, it'll become opt-out once stabilized
        ...
        if (!tryRecoverFatalError) {
            ...
            GFXSTREAM_ABORT(FatalError(ABORT_REASON_OTHER)) << errorMessage;
        }
        ...
        // Mark this render thread as lost and do no allow any more commands
