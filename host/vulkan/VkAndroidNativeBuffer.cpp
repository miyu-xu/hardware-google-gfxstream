...
std::unique_ptr<AndroidNativeBufferInfo> AndroidNativeBufferInfo::create(
    ...
    if (!getColorBufferShareInfo(out->mColorBufferHandle, &colorBufferExportedToGl,
    ...
    if (externalMemoryCompatible) {
        releaseColorBufferForGuestUse(out->mColorBufferHandle);
    ...
    if (out->mExternallyBacked) {
        ...
        if (!importExternalMemory(out->mDeviceDispatch, out->mDevice,
...
VkResult AndroidNativeBufferInfo::on_vkAcquireImageANDROID(VulkanDispatch* vk, VkDevice device,
    ...
    if (mUseVulkanNativeImage) {
        ...
        emu->debugUtilsHelper.cmdBeginDebugLabel(
...
VkResult AndroidNativeBufferInfo::on_vkQueueSignalReleaseImageANDROID(
    gfxstream::host::BackendCallbacks& callbacks, VulkanDispatch* vk, uint32_t queueFamilyIndex,
    ...
    *pNativeFenceFd = -1;
    ...
    if (mUseVulkanNativeImage) {
        ...
        callbacks.flushColorBufferFromBytes(mColorBufferHandle, bytes, bytesSize);
