...
void CompositorVk::setUpDefaultImage() {
    ...
    auto memoryTypeIndexOpt = findMemoryType(imageMemoryRequirements.memoryTypeBits, VK_MEMORY_PROPERTY_DEVICE_LOCAL_BIT);
    ...
    VK_CHECK_MEMALLOC(m_vk.vkAllocateMemory(m_vkDevice, &imageMemoryAllocInfo, nullptr, &imageMemory), imageMemoryAllocInfo);
    ...
    auto [stagingBuffer, stagingBufferMemory] = createStagingBufferWithData(pixels.data(), pixels.size());
    ...
    runSingleTimeCommands(m_vkQueue, m_vkQueueLock, [&, this](const VkCommandBuffer& cmdBuff) {
        ...
        m_vk.vkCmdCopyBufferToImage(cmdBuff,
                                   stagingBuffer,
                                   image,
