   ...
   public:
    VkDecoderTestDispatch(VulkanDispatch* vk, android::base::BumpPool* bp,
        ...
        : mVk(vk),
          ...
          mBoxedHandleManager(&(mDgs->getBoxedHandleManager())),
          mBp(bp),
    ...
    VkResult vkBindBufferMemory(VkDevice device, VkBuffer buffer, VkDeviceMemory memory,
        ...
        return mDgs->on_vkBindBufferMemory(
            mBp, nullptr, device, mBoxedHandleManager->unbox_VkBuffer(buffer),
    ...
    VkResult vkBindImageMemory(VkDevice device, VkImage image, VkDeviceMemory memory,
        ...
        return mDgs->on_vkBindImageMemory(
            mBp, nullptr, device, mBoxedHandleManager->unbox_VkImage(image),
            mBoxedHandleManager->unbox_VkDeviceMemory(memory), memoryOffset);
    }
    ...
    void vkCmdBlitImage(VkCommandBuffer commandBuffer, VkImage srcImage,
        ...
        return mVk->vkCmdBlitImage(mBoxedHandleManager->unbox_VkCommandBuffer(commandBuffer),
                                   mBoxedHandleManager->unbox_VkImage(srcImage), srcImageLayout,
                                   mBoxedHandleManager->unbox_VkImage(dstImage), dstImageLayout,
                                   regionCount, pRegions, filter);
    }
    void vkCmdCopyBufferToImage(VkCommandBuffer commandBuffer, VkBuffer srcBuffer, VkImage dstImage,
                                VkImageLayout dstImageLayout, uint32_t regionCount,
                                const VkBufferImageCopy* pRegions) {
        mDgs->on_vkCmdCopyBufferToImage(mBp, nullptr, commandBuffer,
                                        mBoxedHandleManager->unbox_VkBuffer(srcBuffer),
                                        mBoxedHandleManager->unbox_VkImage(dstImage),
    ...
    void vkCmdCopyImage(VkCommandBuffer commandBuffer, VkImage srcImage,
        ...
        mDgs->on_vkCmdCopyImage(mBp, nullptr, commandBuffer,
                                mBoxedHandleManager->unbox_VkImage(srcImage), srcImageLayout,
                                mBoxedHandleManager->unbox_VkImage(dstImage), dstImageLayout,
    ...
    void vkCmdCopyImageToBuffer(VkCommandBuffer commandBuffer, VkImage srcImage,
        ...
        mDgs->on_vkCmdCopyImageToBuffer(
            mBp, nullptr, commandBuffer, mBoxedHandleManager->unbox_VkImage(srcImage),
            srcImageLayout, mBoxedHandleManager->unbox_VkBuffer(dstBuffer), regionCount, pRegions);
    }
    ...
    void vkDestroyBuffer(VkDevice device, VkBuffer buffer,
                         const VkAllocationCallbacks* pAllocator) {
        mDgs->on_vkDestroyBuffer(mBp, nullptr, device, mBoxedHandleManager->unbox_VkBuffer(buffer),
                                 pAllocator);
    ...
    void vkDestroyCommandPool(VkDevice device, VkCommandPool commandPool,
                              const VkAllocationCallbacks* pAllocator) {
        mDgs->on_vkDestroyCommandPool(mBp, nullptr, device,
                                      mBoxedHandleManager->unbox_VkCommandPool(commandPool),
                                      pAllocator);
    ...
    void vkDestroyImage(VkDevice device, VkImage image, const VkAllocationCallbacks* pAllocator) {
        mDgs->on_vkDestroyImage(mBp, nullptr, device, mBoxedHandleManager->unbox_VkImage(image),
                                pAllocator);
    ...
    void vkDestroyImageView(VkDevice device, VkImageView imageView,
                            const VkAllocationCallbacks* pAllocator) {
        mDgs->on_vkDestroyImageView(mBp, nullptr, device,
                                    mBoxedHandleManager->unbox_VkImageView(imageView), pAllocator);
    ...
    void vkFreeCommandBuffers(VkDevice device, VkCommandPool commandPool,
        ...
        mDgs->on_vkFreeCommandBuffers(mBp, nullptr, device,
                                      mBoxedHandleManager->unbox_VkCommandPool(commandPool),
                                      commandBufferCount, pCommandBuffers);
        // Calling delete_VkCommandBuffer is normally done in the decoder, so we have to do it here.
        for (int i = 0; i < commandBufferCount; ++i) {
            mBoxedHandleManager->delete_VkCommandBuffer(
                mBoxedHandleManager->unboxed_to_boxed_VkCommandBuffer(pCommandBuffers[i]));
        }
    }

    void vkFreeMemory(VkDevice device, VkDeviceMemory memory,
                      const VkAllocationCallbacks* pAllocator) {
        mDgs->on_vkFreeMemory(mBp, nullptr, device,
                              mBoxedHandleManager->unbox_VkDeviceMemory(memory), pAllocator);
    ...
    void vkGetBufferMemoryRequirements(VkDevice device, VkBuffer buffer,
        ...
        mVk->vkGetBufferMemoryRequirements(mBoxedHandleManager->unbox_VkDevice(device),
                                           mBoxedHandleManager->unbox_VkBuffer(buffer),
                                           pMemoryRequirements);
    }
    ...
    void vkGetImageMemoryRequirements(VkDevice device, VkImage image,
                                      VkMemoryRequirements* pMemoryRequirements) {
        mDgs->on_vkGetImageMemoryRequirements(
            mBp, nullptr, device, mBoxedHandleManager->unbox_VkImage(image), pMemoryRequirements);
    }
    ...
    void vkGetPhysicalDeviceQueueFamilyProperties(VkPhysicalDevice physicalDevice,
        ...
        mVk->vkGetPhysicalDeviceQueueFamilyProperties(
            mBoxedHandleManager->unbox_VkPhysicalDevice(physicalDevice), pQueueFamilyPropertyCount,
            pQueueFamilyProperties);
    }

    VkResult vkMapMemory(VkDevice device, VkDeviceMemory memory, VkDeviceSize offset,
                         VkDeviceSize size, VkMemoryMapFlags flags, void** ppData) {
        return mDgs->on_vkMapMemory(mBp, nullptr, device,
                                    mBoxedHandleManager->unbox_VkDeviceMemory(memory), offset, size,
                                    flags, ppData);
    }

    void vkUnmapMemory(VkDevice device, VkDeviceMemory memory) {
        mDgs->on_vkUnmapMemory(mBp, nullptr, device,
                               mBoxedHandleManager->unbox_VkDeviceMemory(memory));
    }
