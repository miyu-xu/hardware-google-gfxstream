                ...
                *readStreamPtrPtr += sizeof(VkSampleCountFlagBits);
                if (latexmath
                    : [\lceil {
                          \mathit { samples }
                          \over 32
                      }\rceil] <= MAX_STACK_ITEMS) {
                    pSampleMask = (VkSampleMask*)stack_pSampleMask;
                } else {
                    readStream->alloc((void**)&pSampleMask, latexmath
                                      : [\lceil {
                                            \mathit { samples }
                                            \over 32
                                        }\rceil] *
                                            sizeof(const VkSampleMask));
                }
                memcpy((VkSampleMask*)pSampleMask, *readStreamPtrPtr, latexmath
                       : [\lceil {
                             \mathit { samples }
                             \over 32
                         }\rceil] *
                             sizeof(const VkSampleMask));
                *readStreamPtrPtr += latexmath : [\lceil {
                                         \mathit { samples }
                                         \over 32
                                     }\rceil] * sizeof(const VkSampleMask);
