...
class VkDecoderSnapshot::Impl {
   public:
        ...
        if (dedicatedAllocateInfo) {
            if (dedicatedAllocateInfo->image) {
                mReconstruction.addHandleDependency(
                    ...
                    (uint64_t)(uintptr_t)m_state->getBoxedHandleManager()->unboxed_to_boxed_non_dispatchable_VkImage(
                        dedicatedAllocateInfo->image));
            }
            if (dedicatedAllocateInfo->buffer) {
                mReconstruction.addHandleDependency(
                    (const uint64_t*)pMemory, 1,
                    (uint64_t)(uintptr_t)m_state->getBoxedHandleManager()->unboxed_to_boxed_non_dispatchable_VkBuffer(
    ...
    void vkBindBufferMemory(android::base::BumpPool* pool, VkSnapshotApiCallInfo* apiCallInfo,
        ...
        VkBuffer boxed_VkBuffer = m_state->getBoxedHandleManager()->unboxed_to_boxed_non_dispatchable_VkBuffer((&buffer)[0]);
        std::lock_guard<std::mutex> lock(mReconstructionMutex);
        // buffer create
        mReconstruction.addHandleDependency(
            (const uint64_t*)&boxed_VkBuffer, 1,
            (uint64_t)(uintptr_t)m_state->getBoxedHandleManager()->unboxed_to_boxed_non_dispatchable_VkDeviceMemory(memory),
    ...
    void vkBindImageMemory(android::base::BumpPool* pool, VkSnapshotApiCallInfo* apiCallInfo,
                           ...
                           VkDeviceMemory memory, VkDeviceSize memoryOffset) {
        VkImage boxed_VkImage = m_state->getBoxedHandleManager()->unboxed_to_boxed_non_dispatchable_VkImage((&image)[0]);
        std::lock_guard<std::mutex> lock(mReconstructionMutex);
        // image create
        mReconstruction.addHandleDependency(
    ...
    void vkCreateImageView(android::base::BumpPool* pool, VkSnapshotApiCallInfo* apiCallInfo,
        ...
        mReconstruction.addHandleDependency(
            ...
            (uint64_t)(uintptr_t)m_state->getBoxedHandleManager()->unboxed_to_boxed_non_dispatchable_VkImage(pCreateInfo->image),
        ...
        for (uint32_t i = 0; i < createInfoCount; ++i) {
            for (uint32_t j = 0; j < pCreateInfos[i].stageCount; ++j) {
                mReconstruction.addHandleDependency(
                    ...
                    (uint64_t)(uintptr_t)m_state->getBoxedHandleManager()->unboxed_to_boxed_non_dispatchable_VkShaderModule(
                        pCreateInfos[i].pStages[j].module));
            }
            mReconstruction.addHandleDependency(
                (const uint64_t*)(pPipelines + i), 1,
                (uint64_t)(uintptr_t)m_state->getBoxedHandleManager()->unboxed_to_boxed_non_dispatchable_VkRenderPass(
    ...
    void vkAllocateDescriptorSets(android::base::BumpPool* pool, VkSnapshotApiCallInfo* apiCallInfo,
        ...
        mReconstruction.addHandleDependency(
            ...
            (uint64_t)(uintptr_t)m_state->getBoxedHandleManager()->unboxed_to_boxed_non_dispatchable_VkDescriptorPool(
...
//        VkDecoderGlobalState* m_state = VkDecoderGlobalState::get();
        ...
        for (uint32_t i = 0; i < descriptorWriteCount; ++i) {
            mReconstruction.addHandleDependency(
                ...
                (uint64_t)(uintptr_t)m_state->getBoxedHandleManager()->unboxed_to_boxed_non_dispatchable_VkDescriptorSet(
            ...
            for (uint32_t j = 0; j < pDescriptorWrites[i].descriptorCount; ++j) {
                if ((pDescriptorWrites[i].pImageInfo)) {
                    if (pDescriptorWrites[i].descriptorType ==
                        ...
                        mReconstruction.addHandleDependency(
                            (const uint64_t*)(&handle), 1,
                            (uint64_t)(uintptr_t)m_state->getBoxedHandleManager()->unboxed_to_boxed_non_dispatchable_VkSampler(
                                pDescriptorWrites[i].pImageInfo[j].sampler));
                        mReconstruction.addHandleDependency(
                            (const uint64_t*)(&handle), 1,
                            (uint64_t)(uintptr_t)m_state->getBoxedHandleManager()->unboxed_to_boxed_non_dispatchable_VkImageView(
    ...
    void vkAllocateCommandBuffers(android::base::BumpPool* pool, VkSnapshotApiCallInfo* apiCallInfo,
        ...
        mReconstruction.addHandleDependency(
            ...
            (uint64_t)(uintptr_t)m_state->getBoxedHandleManager()->unboxed_to_boxed_non_dispatchable_VkCommandPool(
    ...
    void vkBindImageMemory2(android::base::BumpPool* pool, VkSnapshotApiCallInfo* apiCallInfo,
        ...
        for (uint32_t i = 0; i < bindInfoCount; ++i) {
            VkImage boxed_VkImage = m_state->getBoxedHandleManager()->unboxed_to_boxed_non_dispatchable_VkImage(pBindInfos[i].image);
            VkDeviceMemory boxed_VkDeviceMemory =
                m_state->getBoxedHandleManager()->unboxed_to_boxed_non_dispatchable_VkDeviceMemory(pBindInfos[i].memory);
            mReconstruction.addHandleDependency((const uint64_t*)&boxed_VkImage, 1,
    ...
    void vkMapMemoryIntoAddressSpaceGOOGLE(android::base::BumpPool* pool,
        ...
        mReconstruction.setApiTrace(apiCallInfo, apiCallPacket, apiCallPacketSize);
        for (uint32_t i = 0; i < 1; ++i) {
            VkDeviceMemory boxed = m_state->getBoxedHandleManager()->unboxed_to_boxed_non_dispatchable_VkDeviceMemory((&memory)[i]);
            mReconstruction.forEachHandleAddModifyApi((const uint64_t*)(&boxed), 1, apiCallHandle);
        }
    }
