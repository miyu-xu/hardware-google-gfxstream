...
TEST(VkGuestQueueUtilsTest, Passthrough) {
    ...
    EmulatedPhysicalDeviceQueueProperties helper(hostMemoryProperties, 1, features);

    // Passthrough when no features enabled:
    const auto actualQueueProperties = helper.getQueueFamilyProperties();
    EXPECT_THAT(actualQueueProperties, EqsVkQueueFamilyProperties(hostQueueFamilyProperties));
}
...
TEST(VkGuestQueueUtilsTest, VulkanVirtualQueue) {
    ...
    const auto actualQueueProperties = helper.getQueueFamilyProperties();
    EXPECT_THAT(actualQueueProperties, EqsVkQueueFamilyProperties(expectedQueueFamilyProperties));
}
