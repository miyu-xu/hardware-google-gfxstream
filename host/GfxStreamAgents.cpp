...
static const QAndroidMultiDisplayAgent sMultiDisplayAgent = {
    .setMultiDisplay = [](uint32_t id, int32_t x, int32_t y, uint32_t w, uint32_t h, uint32_t dpi,
                          uint32_t flag, bool add) -> int { return 0; },
    .getMultiDisplay = [](uint32_t id, int32_t* x, int32_t* y, uint32_t* w, uint32_t* h,
                          uint32_t* dpi, uint32_t* flag, bool* enabled) -> bool {
    ...
    .getNextMultiDisplay = [](int32_t start_id, uint32_t* id, int32_t* x, int32_t* y, uint32_t* w,
                              uint32_t* h, uint32_t* dpi, uint32_t* flag, uint32_t* cb) -> bool {
        uint32_t key;
    ...
    .isMultiDisplayEnabled = [](void) -> bool { return mMultiDisplay.size() > 1; },
    .getCombinedDisplaySize = [](uint32_t* width, uint32_t* height) {},
    .multiDisplayParamValidate = [](uint32_t id, uint32_t w, uint32_t h, uint32_t dpi,
    ...
    .createDisplay = [](uint32_t* displayId) -> int {
        ...
        if (*displayId == MultiDisplay::s_invalidIdMultiDisplay) {
            for (int i = MultiDisplay::s_displayIdInternalBegin;
                 i < MultiDisplay::s_maxNumMultiDisplay; i++) {
    ...
    .setDisplayPose = [](uint32_t displayId, int32_t x, int32_t y, uint32_t w, uint32_t h,
                         uint32_t dpi) -> int {
        if (mMultiDisplay.find(displayId) == mMultiDisplay.end()) {
    ...
    .getDisplayPose = [](uint32_t displayId, int32_t* x, int32_t* y, uint32_t* w,
                         uint32_t* h) -> int {
        if (mMultiDisplay.find(displayId) == mMultiDisplay.end()) {
            fprintf(stderr, "cannot find display %d\n", displayId);
            return -1;
        }
        if (x) *x = mMultiDisplay[displayId].pos_x;
        if (y) *y = mMultiDisplay[displayId].pos_y;
        if (w) *w = mMultiDisplay[displayId].width;
        if (h) *h = mMultiDisplay[displayId].height;
        return 0;
    },
    .getDisplayColorBuffer = [](uint32_t displayId, uint32_t* colorBuffer) -> int {
        if (mMultiDisplay.find(displayId) == mMultiDisplay.end()) {
            fprintf(stderr, "cannot find display %d\n", displayId);
            return -1;
        }
    ...
    .getColorBufferDisplay = [](uint32_t colorBuffer, uint32_t* displayId) -> int {
    ...
    .setDisplayColorBuffer = [](uint32_t displayId, uint32_t colorBuffer) -> int {
        if (mMultiDisplay.find(displayId) == mMultiDisplay.end()) {
            fprintf(stderr, "cannot find display %d\n", displayId);
            return -1;
        }
...
static const QAndroidEmulatorWindowAgent sQAndroidEmulatorWindowAgent = {
    ...
    .getRotation =
        [](void) {
            DEBUG_LOG(stderr, "window-agent-GfxStream-impl: .getRotation\n");
            return (int)SKIN_ROTATION_0;
        },
    .showMessage =
        [](const char* message, WindowMessageType type, int timeoutMs) {
            DEBUG_LOG(stderr, "window-agent-GfxStream-impl: .showMessage %s\n", message);
        },
    .showMessageWithDismissCallback =
        [](const char* message, WindowMessageType type, const char* dismissText, void* context,
    ...
    .setNoSkin = [](void) {},
    .restoreSkin = [](void) {},
    .updateUIMultiDisplayPage = [](uint32_t id) { DEBUG_LOG(stderr, "updateMultiDisplayPage\n"); },
...
static const QAndroidVmOperations sQAndroidVmOperations = {
    ...
    .setSkipSnapshotSave =
        [](bool used) { DEBUG_LOG(stderr, "goldfish-opengl vm ops: set skip snapshot save\n"); },
...
const QAndroidVmOperations* const GfxStreamGraphicsAgentFactory::android_get_QAndroidVmOperations()
    const {
...
GfxStreamGraphicsAgentFactory::android_get_QAndroidEmulatorWindowAgent() const {
