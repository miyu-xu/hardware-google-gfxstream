...
class TestingANW {
  public:
    TestingANW(uint32_t width,
        ...
        for (auto& buffer : mBuffers) {
            mBufferQueue.push_back(buffer.get());
        }
    }
    ...
    int queueBuffer(EGLClientBuffer buffer, int) {
        auto ahb = reinterpret_cast<TestingAHB*>(buffer);
        mBufferQueue.push_back(ahb);
        return 0;
    }

    int dequeueBuffer(EGLClientBuffer* buffer, int* fence) {
        auto ahb = mBufferQueue.front();
        mBufferQueue.pop_front();

        *buffer = ahb->asEglClientBuffer();
        *fence = -1;
        return 0;
    }

    int cancelBuffer(EGLClientBuffer) {
        return 0;
    }
  ...
  private:
    ...
    std::deque<TestingAHB*> mBufferQueue;
};
...
TEST_P(GfxstreamEnd2EndGlTest, BasicViewport) {
    ...
    GLint viewport[4] = {};
    mGles2->glGetIntegerv(GL_VIEWPORT, viewport);
...
TEST_P(GfxstreamEnd2EndGlTest, NativeWindow) {
    ...
    EGLSurface surface = mEgl->eglCreateWindowSurface(display, config, anwEgl, nullptr);
    ASSERT_THAT(surface, Not(Eq(EGL_NO_SURFACE)));
    ...
    for (int i = 0; i < iterations; i++) {
        ...
        mGles2->glClearColor(1.0f, 0.0f, static_cast<float>(i) / static_cast<float>(iterations), 1.0f);
