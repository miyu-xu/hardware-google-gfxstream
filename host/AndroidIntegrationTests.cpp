...
TEST_F(AndroidIntegrationTest, VulkanImportAHB) {
    ...
    VkPhysicalDevice physicalDevice = VK_NULL_HANDLE;
    {
        ...
        ASSERT_THAT(guest.vk.vkEnumeratePhysicalDevices(instance, &physicalDeviceCount, nullptr), Eq(VK_SUCCESS));
        ASSERT_THAT(physicalDeviceCount, Gt(0));

        std::vector<VkPhysicalDevice> physicalDevices(physicalDeviceCount);
        ASSERT_THAT(guest.vk.vkEnumeratePhysicalDevices(instance, &physicalDeviceCount, physicalDevices.data()), Eq(VK_SUCCESS));
        for (VkPhysicalDevice physicalDevice : physicalDevices) {
            VkPhysicalDeviceProperties physicalDeviceProperties = {};
            guest.vk.vkGetPhysicalDeviceProperties(physicalDevice, &physicalDeviceProperties);
            ALOGI("Found physical device: %s", physicalDeviceProperties.deviceName);
        }
    ...
    VkDevice device = VK_NULL_HANDLE;
    {
        ...
        ASSERT_THAT(guest.vk.vkCreateDevice(physicalDevice, &dci, nullptr, &device), Eq(VK_SUCCESS));
        ASSERT_THAT(device, Not(Eq(VK_NULL_HANDLE)));
    }
    ...
    VkImage image = VK_NULL_HANDLE;
    {
        ...
        ASSERT_THAT(guest.vk.vkCreateImage(device, &ici, nullptr, &image), Eq(VK_SUCCESS));
        ASSERT_THAT(image, Not(Eq(VK_NULL_HANDLE)));
    }

    VkDeviceSize imageMemorySize = 0;
    {
        VkMemoryRequirements imageMemoryRequirements = {};
        guest.vk.vkGetImageMemoryRequirements(device, image, &imageMemoryRequirements);
    }

    VkDeviceMemory imageMemory = VK_NULL_HANDLE;
    {
        const VkImportAndroidHardwareBufferInfoANDROID iahbi = {
            .sType = VK_STRUCTURE_TYPE_IMPORT_ANDROID_HARDWARE_BUFFER_INFO_ANDROID,
            .pNext = nullptr,
            .buffer = ahb->asAHardwareBuffer(),
        };
        const VkMemoryDedicatedAllocateInfo mdai = {
        ...
        const VkMemoryAllocateInfo mai = {
            ...
            .pNext = &mdai,
            .allocationSize = imageMemorySize,
            .memoryTypeIndex = 0,
        };

        ASSERT_THAT(guest.vk.vkAllocateMemory(device, &mai, nullptr, &imageMemory), Eq(VK_SUCCESS));
    }
    ASSERT_THAT(guest.vk.vkBindImageMemory(device, image, imageMemory, 0), Eq(VK_SUCCESS));
    */

    auto gfxstreamGuestGetInstanceProcAddr =
        reinterpret_cast<PFN_vkGetInstanceProcAddr>(
            GfxstreamGuestVulkanGetInstanceProcAddrForTesting());

    ::vk::DynamicLoader loader;
    VULKAN_HPP_DEFAULT_DISPATCHER.init(gfxstreamGuestGetInstanceProcAddr);
    ...
    auto physical_devices = VK_EXPECT(::vk::raii::PhysicalDevices::create(instance))
}
