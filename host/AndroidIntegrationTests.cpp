  ...
  protected:
    void SetUp() override {
        ...
        std::vector<stream_renderer_param> renderer_params{
            {STREAM_RENDERER_PARAM_USER_DATA, static_cast<uint64_t>(reinterpret_cast<uintptr_t>(mDevice.get()))},
        ...
        mVk = std::make_unique<::vk::raii::Context>(
    ...
    std::unique_ptr<::vk::raii::Context> mVk;
};

namespace {

uint32_t GetMemoryType(const ::vk::raii::PhysicalDevice& physicalDevice,
...
TEST_F(AndroidIntegrationTest, VulkanImportAHB) {
    ...
    auto queue = ::vk::raii::Queue(device, graphicsQueueFamilyIndex, 0);

    const uint32_t width = 32;
    const uint32_t height = 32;
    auto ahb = mGralloc->allocate(width, height, DRM_FORMAT_ABGR8888);

    const ::vk::NativeBufferANDROID imageNativeBufferInfo = {
    ...
    const uint32_t imageMemoryIndex =
        GetMemoryType(physicalDevice, imageMemoryRequirements, ::vk::MemoryPropertyFlagBits::eDeviceLocal);
    ...
    auto stagingBuffer = VK_EXPECT(::vk::raii::Buffer::create(device, bufferCreateInfo));
    ...
    const ::vk::MemoryAllocateInfo stagingBufferMemoryAllocateInfo = {
        .allocationSize = stagingBufferMemoryRequirements.size,
        .memoryTypeIndex = stagingBufferMemoryType,
    };
    auto stagingBufferMemory = VK_EXPECT(::vk::raii::DeviceMemory::create(device, stagingBufferMemoryAllocateInfo));
    stagingBuffer.bindMemory(*stagingBufferMemory, 0);

    const ::vk::CommandPoolCreateInfo commandPoolCreateInfo = {
        .queueFamilyIndex = graphicsQueueFamilyIndex,
    };
    auto commandPool = VK_EXPECT(::vk::raii::CommandPool::create(device, commandPoolCreateInfo));
    ...
    auto commandBuffers = VK_EXPECT(::vk::raii::CommandBuffers::create(device, commandBufferAllocateInfo));
    ...
    auto transferFence = VK_EXPECT(::vk::raii::Fence::create(device, ::vk::FenceCreateInfo()));
    ...
    ASSERT_THAT(waitResult, Eq(::vk::Result::eSuccess));
