...
void ExternalObjectManager::addResourceExternalHandleInfo(uint32_t resHandle, const ExternalHandleInfo& externalHandleInfo) {
    std::lock_guard<std::mutex> lock(mLock);
    mResourceExternalHandleInfos.insert(std::make_pair(resHandle, externalHandleInfo));
}

std::optional<ExternalHandleInfo> ExternalObjectManager::removeResourceExternalHandleInfo(uint32_t resHandle) {
    std::lock_guard<std::mutex> lock(mLock);
