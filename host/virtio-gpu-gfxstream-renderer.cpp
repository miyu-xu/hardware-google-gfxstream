...
static int stream_renderer_opengles_init(uint32_t display_width, uint32_t display_height,
    ...
    for (const std::string& renderer_feature : android::base::Split(renderer_features, ",")) {
        if (renderer_feature.empty()) continue;

        const std::vector<std::string>& parts = android::base::Split(renderer_feature, ":");
        if (parts.size() != 2) {
            stream_renderer_error("Error: invalid renderer features: %s", renderer_features.c_str());
            return -EINVAL;
        }
        ...
        stream_renderer_error("Gfxstream feature %s %s", feature_name.c_str(), feature_status.c_str());
    }
...
VG_EXPORT int stream_renderer_init(struct stream_renderer_param* stream_renderer_params,
    ...
    for (uint64_t i = 0; i < num_params; ++i) {
        ...
        switch (param.key) {
            ...
            case STREAM_RENDERER_PARAM_RENDERER_FEATURES: {
                renderer_features = std::string(reinterpret_cast<const char*>(static_cast<uintptr_t>(param.value)));
                break;
            }
    ...
    if (!skip_opengles) {
        ...
        int ret = stream_renderer_opengles_init(display_width, display_height, renderer_flags, renderer_features);
        if (ret) {
            return ret;
        }
    }
