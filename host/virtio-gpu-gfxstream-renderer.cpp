...
void stream_renderer_debug(uint32_t type, const char* file, int line, const char* format, ...) {
    char buf[MAX_DEBUG_BUFFER_SIZE];
    int bytes_written = snprintf(buf, sizeof(buf), "[%s(%d)] ", basename(file), line);
    if (bytes_written >= 0 && bytes_written < MAX_DEBUG_BUFFER_SIZE) {
        va_list args;
        va_start(args, format);
        vsnprintf(buf + bytes_written, MAX_DEBUG_BUFFER_SIZE - bytes_written, format, args);
        va_end(args);
    }
...
#if STREAM_RENDERER_DEBUG_LEVEL >= 1
#define stream_renderer_error(format, ...)                                             \
    do {                                                                               \
        stream_renderer_debug(STREAM_RENDERER_DEBUG_ERROR, __FILE__, __LINE__, format, \
                              ##__VA_ARGS__);                                          \
    } while (0)
#else
#define stream_renderer_error(format, ...)
...
#if STREAM_RENDERER_DEBUG_LEVEL >= 3
#define stream_renderer_info(format, ...)                                             \
    do {                                                                              \
        stream_renderer_debug(STREAM_RENDERER_DEBUG_INFO, __FILE__, __LINE__, format, \
                              ##__VA_ARGS__);                                         \
    } while (0)
#else
#define stream_renderer_info(format, ...)
