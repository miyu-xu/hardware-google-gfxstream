...
#define PIPE_BIND_QUERY_BUFFER (1 << 18)        /* get_query_result_resource */
    ...
    int platformResourceInfo(int res_handle, int* width, int* height, int* internal_format) {
        auto it = mResources.find(res_handle);
        if (it == mResources.end()) return -EINVAL;
        bool success = gfxstream::FrameBuffer::getFB()->getColorBufferInfo(res_handle, width,
                                                                           height, internal_format);
        return success ? 0 : -1;
    }

    void* platformCreateSharedEglContext() {
        return gfxstream::FrameBuffer::getFB()->platformCreateSharedEglContext();
    }

    int platformDestroySharedEglContext(void* context) {
        bool success = gfxstream::FrameBuffer::getFB()->platformDestroySharedEglContext(context);
        return success ? 0 : -1;
    }
