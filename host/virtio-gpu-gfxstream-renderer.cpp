    ...
    void fillCaps(uint32_t set, void* caps) {
        switch (set) {
            case VIRTGPU_CAPSET_GFXSTREAM_VULKAN: {
                ...
                memset(capset->virglSupportedFormats, 0xFF, sizeof(capset->virglSupportedFormats));
                set_drm_format_supported(capset->virglSupportedFormats, DRM_FORMAT_DEPTH16, 0);
                set_drm_format_supported(capset->virglSupportedFormats, DRM_FORMAT_DEPTH24, 0);
                set_drm_format_supported(capset->virglSupportedFormats, DRM_FORMAT_DEPTH32, 0);
                set_drm_format_supported(capset->virglSupportedFormats, DRM_FORMAT_DEPTH24_STENCIL8,
                                         0);
                set_drm_format_supported(capset->virglSupportedFormats, DRM_FORMAT_DEPTH32_STENCIL8,
                                         0);

                // Only allow depth/stencil if GLES passthrough is not active
                if (mFeatures.GuestVulkanOnly.enabled && vk_emu && vk_emu->live) {
                    for (size_t i = 0; i < vk_emu->imageSupportInfo.size(); ++i) {
                        if (gfxstream::vk::formatIsDepthOrStencil(
                                vk_emu->imageSupportInfo[i].format) &&
                            vk_emu->imageSupportInfo[i].supported &&
                            vk_emu->imageSupportInfo[i]
                                    .formatProps2.formatProperties.optimalTilingFeatures &
                                VK_FORMAT_FEATURE_DEPTH_STENCIL_ATTACHMENT_BIT) {
                            switch (vk_emu->imageSupportInfo[i].format) {
                                case VK_FORMAT_D16_UNORM:
                                    set_drm_format_supported(capset->virglSupportedFormats,
                                                             DRM_FORMAT_DEPTH16, 1);
                                    break;
                                case VK_FORMAT_X8_D24_UNORM_PACK32:
                                    set_drm_format_supported(capset->virglSupportedFormats,
                                                             DRM_FORMAT_DEPTH24, 1);
                                    break;
                                case VK_FORMAT_D24_UNORM_S8_UINT:
                                    set_drm_format_supported(capset->virglSupportedFormats,
                                                             DRM_FORMAT_DEPTH24_STENCIL8, 1);
                                    break;
                                case VK_FORMAT_D32_SFLOAT:
                                    set_drm_format_supported(capset->virglSupportedFormats,
                                                             DRM_FORMAT_DEPTH32, 1);
                                    break;
                                case VK_FORMAT_D32_SFLOAT_S8_UINT:
                                    set_drm_format_supported(capset->virglSupportedFormats,
                                                             DRM_FORMAT_DEPTH32_STENCIL8, 1);
                                    break;
                                default:
                                    break;
                            }
