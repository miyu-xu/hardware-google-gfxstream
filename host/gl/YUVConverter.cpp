...
static void getYUVOffsets(int width, int height, FrameworkFormat format, bool yuv420888ToNv21,
                          uint32_t* yWidth, uint32_t* yHeight, uint32_t* yOffsetBytes,
                          uint32_t* yStridePixels, uint32_t* yStrideBytes, uint32_t* uWidth,
                          uint32_t* uHeight, uint32_t* uOffsetBytes, uint32_t* uStridePixels,
                          uint32_t* uStrideBytes, uint32_t* vWidth, uint32_t* vHeight,
                          uint32_t* vOffsetBytes, uint32_t* vStridePixels, uint32_t* vStrideBytes) {
    switch (format) {
        case FRAMEWORK_FORMAT_YV12: {
            *yWidth = width;
            *yHeight = height;
...
void YUVConverter::createYUVGLTex(GLenum textureUnit, GLsizei width, GLsizei height,
...
static void readYUVTex(GLuint tex, FrameworkFormat format, bool yuv420888ToNv21, YUVPlane plane,
                       void* pixels, uint32_t pixelsStride) {
...
static void subUpdateYUVGLTex(GLenum texture_unit, GLuint tex, int x, int y, int width, int height,
                              FrameworkFormat format, bool yuv420888ToNv21, YUVPlane plane,
                              const void* pixels) {
...
void YUVConverter::init(int width, int height, FrameworkFormat format) {
    ...
    getYUVOffsets(width, height, mFormat, mYuv420888ToNv21, &yWidth, &yHeight, &yOffsetBytes,
                  &yStridePixels, &yStrideBytes, &uWidth, &uHeight, &uOffsetBytes, &uStridePixels,
                  &uStrideBytes, &vWidth, &vHeight, &vOffsetBytes, &vStridePixels, &vStrideBytes);
    mWidth = width;
    mHeight = height;
    if (!mTextureY) {
        createYUVGLTex(GL_TEXTURE0, yStridePixels, yHeight, mFormat, mYuv420888ToNv21, YUVPlane::Y,
                       &mTextureY);
    }
    if (isInterleaved(mFormat, mYuv420888ToNv21)) {
        if (!mTextureU) {
            createYUVGLTex(GL_TEXTURE1, uStridePixels, uHeight, mFormat, mYuv420888ToNv21,
                           YUVPlane::UV, &mTextureU);
            mTextureV = mTextureU;
        }
    } else {
        if (!mTextureU) {
            createYUVGLTex(GL_TEXTURE1, uStridePixels, uHeight, mFormat, mYuv420888ToNv21,
                           YUVPlane::U, &mTextureU);
        }
        if (!mTextureV) {
            createYUVGLTex(GL_TEXTURE2, vStridePixels, vHeight, mFormat, mYuv420888ToNv21,
                           YUVPlane::V, &mTextureV);
        }
    }
...
void YUVConverter::readPixels(uint8_t* pixels, uint32_t pixels_size) {
    ...
    getYUVOffsets(mWidth, mHeight, mFormat, mYuv420888ToNv21, &yWidth, &yHeight, &yOffsetBytes,
                  &yStridePixels, &yStrideBytes, &uWidth, &uHeight, &uOffsetBytes, &uStridePixels,
                  &uStrideBytes, &vWidth, &vHeight, &vOffsetBytes, &vStridePixels, &vStrideBytes);

    if (isInterleaved(mFormat, mYuv420888ToNv21)) {
        readYUVTex(mTextureV, mFormat, mYuv420888ToNv21, YUVPlane::UV,
                   pixels + std::min(uOffsetBytes, vOffsetBytes), uStridePixels);
    } else {
        readYUVTex(mTextureU, mFormat, mYuv420888ToNv21, YUVPlane::U, pixels + uOffsetBytes,
                   uStridePixels);
        readYUVTex(mTextureV, mFormat, mYuv420888ToNv21, YUVPlane::V, pixels + vOffsetBytes,
                   vStridePixels);
    }
    ...
    readYUVTex(mTextureY, mFormat, mYuv420888ToNv21, YUVPlane::Y, pixels + yOffsetBytes,
               yStridePixels);
}
...
void YUVConverter::drawConvertFromFormat(FrameworkFormat format, int x, int y, int width,
    ...
    if (pixels) {
        subUpdateYUVGLTex(GL_TEXTURE0, mTextureY, x, y, yStridePixels, yHeight, mFormat,
                          mYuv420888ToNv21, YUVPlane::Y, pixels + yOffsetBytes);
        if (isInterleaved(mFormat, mYuv420888ToNv21)) {
            subUpdateYUVGLTex(GL_TEXTURE1, mTextureU, x, y, uStridePixels, uHeight, mFormat,
                              mYuv420888ToNv21, YUVPlane::UV,
                              pixels + std::min(uOffsetBytes, vOffsetBytes));
        } else {
            subUpdateYUVGLTex(GL_TEXTURE1, mTextureU, x, y, uStridePixels, uHeight, mFormat,
                              mYuv420888ToNv21, YUVPlane::U, pixels + uOffsetBytes);
            subUpdateYUVGLTex(GL_TEXTURE2, mTextureV, x, y, vStridePixels, vHeight, mFormat,
                              mYuv420888ToNv21, YUVPlane::V, pixels + vOffsetBytes);
        }
    } else {
