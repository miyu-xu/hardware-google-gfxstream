...
#endif
static const uint32_t VK_INSTANCE_CREATE_ENUMERATE_PORTABILITY_BIT_KHR = 0x00000001;
#endif
#endif
...
static void resetBackendList(int bitness) {
    delete sBackendList;
    if (android::base::getEnvironmentVariable("ANDROID_EMUGL_FIXED_BACKEND_LIST") == "1") {
        ...
        sBackendList = new EmuglBackendList(android::base::getLauncherDirectory().c_str(), bitness);
    }
}

static bool stringVectorContains(const std::vector<std::string>& list, const char* value) {
...
bool isHostGpuBlacklisted() { return async_query_host_gpu_blacklisted(); }
...
SelectedRenderer emuglConfig_get_renderer(const char* gpu_mode) {
    ...
    } else if (!strcmp(gpu_mode, "angle") || !strcmp(gpu_mode, "swangle")) {
    ...
    } else if (!strcmp(gpu_mode, "angle_indirect") || !strcmp(gpu_mode, "swangle_indirect")) {
...
static SelectedRenderer sCurrentRenderer = SELECTED_RENDERER_UNKNOWN;

SelectedRenderer emuglConfig_get_current_renderer() { return sCurrentRenderer; }

static std::string sGpuOption;

const char* emuglConfig_get_user_gpu_option() { return sGpuOption.c_str(); }
...
int getSelectedGpuIndex(const std::vector<DeviceSupportInfo>& deviceInfos) {
    ...
    if (enforcedGpuStr.size()) {
        ...
        if (enforceGpuIndex >= 0 && enforceGpuIndex < (int)deviceInfos.size()) {
...
const char* emuglConfig_get_vulkan_runtime_full_path() {
    ...
    if (vkRuntimeOption == "SYSTEM") {
        ...
        VERBOSE("%s: Checking for %s versions", __func__, myLibName);
        if (android::base::queryFileVersionInfo(systemVkRuntimePath.c_str(), &globalMajor,
            ...
            VERBOSE("%s version: %d.%d.%d.%d", systemVkRuntimePath.c_str(), globalMajor, globalMinor,
                   globalBuild_1, globalBuild_2);
            VERBOSE("%s version: %d.%d.%d.%d", localVkRuntimePath.c_str(), localMajor, localMinor,
    ...
    VERBOSE("%s: Using vulkan runtime path: %s", __func__, sVkRuntimePath.c_str());
...
void emuglConfig_get_vulkan_hardware_gpu(char** vendor, int* major, int* minor, int* patch,
    ...
    if (!vendor || !major || !minor || !patch) {
        ERR("%s: Invalid argument!", __func__);
        return;
    }
...
bool emuglConfig_get_vulkan_hardware_gpu_support_info(DeviceSupportInfo* outProps) {
    ...
    if (!library) {
        WARN("%s: cannot open vulkan lib %s\n", __func__, mylibname);
        return false;
    }
    ...
    if (!library) {
        WARN("%s: failed to open %s", __func__, mylibname);
        return false;
    }
    auto* pvkGetInstanceProcAddr =
        reinterpret_cast<PFN_vkGetInstanceProcAddr>(dlsym(library, "vkGetInstanceProcAddr"));
#endif

    if (!pvkGetInstanceProcAddr) {
        ERR("Failed to load vkGetInstanceProcAddr function!");
        return false;
    }

#define GET_VK_INSTANCE_PROC(inst, name) PFN_##name(pvkGetInstanceProcAddr(inst, #name));

    auto* pvkCreateInstance = GET_VK_INSTANCE_PROC(nullptr, vkCreateInstance);
    if (!pvkCreateInstance) {
        ERR("Failed to load vkCreateInstance function!");
        return false;
    }
    ...
    if (result != VK_SUCCESS) {
        ERR("%s: Failed to create vulkan instance. Error: [%s] %d\n", __func__,
               string_VkResult(result), result);
        return false;
    }
    VERBOSE("%s: Successfully created vulkan instance\n", __func__);

    auto* pvkDestroyInstance = GET_VK_INSTANCE_PROC(instance, vkDestroyInstance);
    auto* pvkEnumeratePhysicalDevices = GET_VK_INSTANCE_PROC(instance, vkEnumeratePhysicalDevices);
...
#undef GET_VK_INSTANCE_PROC
    ...
    if (!pvkEnumeratePhysicalDevices || !pvkGetPhysicalDeviceProperties || !pvkDestroyInstance ||
        !pvkGetPhysicalDeviceMemoryProperties || !pvkGetPhysicalDeviceQueueFamilyProperties) {
        ERR("Failed to load Vulkan functions!");
        return false;
    }
    ...
    if (result != VK_SUCCESS) {
        ...
        ERR("%s: Failed to query physical devices count. Error: %s [%d]\n", __func__,
               string_VkResult(result), result);
        return false;
    }
    VERBOSE("%s: Physical devices count is %d\n", __func__, (int)(deviceCount));
    if (deviceCount == 0) {
        pvkDestroyInstance(instance, nullptr);
        ERR(
            "%s: Could not find any Vulkan supported devices, try updating "
            "your GPU drivers.\n",
            __func__);
        return false;
    }
    ...
    if (result != VK_SUCCESS) {
        pvkDestroyInstance(instance, nullptr);
        ERR("%s: Failed to query physical devices. Error: %s [%d]\n", __func__,
               string_VkResult(result), result);
        return false;
    }
    ...
    for (int i = 0; i < (int)deviceCount; i++) {
        pvkGetPhysicalDeviceProperties(devices[i], &deviceInfos[i].physdevProps);

        pvkGetPhysicalDeviceMemoryProperties(devices[i], &deviceInfos[i].memProperties);

        deviceInfos[i].hasGraphicsQueueFamily = false;
        {
            uint32_t queueFamilyCount = 0;
            pvkGetPhysicalDeviceQueueFamilyProperties(devices[i], &queueFamilyCount, nullptr);
            std::vector<VkQueueFamilyProperties> queueFamilyProps(queueFamilyCount);
            pvkGetPhysicalDeviceQueueFamilyProperties(devices[i], &queueFamilyCount,
        ...
        // errors more easily
        const VkPhysicalDeviceProperties& physdevProps = deviceInfos[i].physdevProps;
        const char* deviceType = string_VkPhysicalDeviceType(physdevProps.deviceType);
        ...
        INFO(
            "%s: Found physical GPU '%s', type: %s, apiVersion: %d.%d.%d, "
            "driverVersion: %s\n",
            __func__, physdevProps.deviceName, deviceType, vkMajor, vkMinor, vkPatch,
...
bool emuglConfig_init(EmuglConfig* config, bool gpu_enabled, const char* gpu_mode,
                      const char* gpu_option, int bitness, bool no_window, bool blacklisted,
                      bool has_guest_renderer, int uiPreferredBackend, bool use_host_vulkan) {
    D("%s: blacklisted=%d has_guest_renderer=%d, mode: %s, option: %s\n", __FUNCTION__, blacklisted,
      has_guest_renderer, gpu_mode, gpu_option);
    ...
    bool hasUiPreference =
        (enum WinsysPreferredGlesBackend)uiPreferredBackend != WINSYS_GLESBACKEND_PREFERENCE_AUTO;
    ...
    if (gpu_option) {
        ...
        } else if (!strcmp(gpu_option, "off") || !strcmp(gpu_option, "disable") ||
        ...
        if (gpu_enabled && gpu_mode &&
            (!strcmp(gpu_mode, "on") || !strcmp(gpu_mode, "enable") || !strcmp(gpu_mode, "host"))) {
            gpu_enabled = true;
            gpu_mode = "host";
    ...
    if (gpu_mode && (!strcmp(gpu_mode, "guest") || !strcmp(gpu_mode, "off"))) {
        gpu_enabled = false;
    }
    ...
    if (!gpu_enabled) {
        ...
        snprintf(config->status, sizeof(config->status), "GPU emulation is disabled");
    ...
    if (!bitness) {
        bitness = android::base::getProgramBitness();
    }
    ...
    if (!strcmp(gpu_mode, "auto") || host_set_in_hwconfig) {
        ...
        if (android::base::isRemoteSession(&sessionType)) {
            D("%s: %s session detected\n", __FUNCTION__, sessionType.c_str());
            if (swangle_backend_name && sBackendList->contains(swangle_backend_name)) {
            ...
            if (swangle_backend_name &&
                stringVectorContains(sBackendList->names(), swangle_backend_name)) {
                D("%s: Headless mode or blacklisted GPU driver, "
                  "using SwANGLE backend\n",
                  __FUNCTION__);
                gpu_mode = "swangle_indirect";
            } else if (stringVectorContains(sBackendList->names(), swiftshader_backend_name)) {
                D("%s: Headless mode or blacklisted GPU driver, "
                ...
                gpu_mode = "off";
                snprintf(config->backend, sizeof(config->backend), "%s", gpu_mode);
                snprintf(config->status, sizeof(config->status), "GPU emulation is in the guest");
                gpu_mode = "guest";
                setCurrentRenderer(gpu_mode);
                return true;
            }
    ...
    const bool is_xr_mode = false;  // TODO
    ...
    bool force_swiftshader = (!strcmp("angle", gpu_mode) || !strcmp("angle_indirect", gpu_mode) ||
                              !strcmp("angle9", gpu_mode) || !strcmp("angle9_indirect", gpu_mode));
#ifdef _WIN32
    // Also turn off swangle_indirect mode on Windows
    force_swiftshader = force_swiftshader || (!strcmp("swangle_indirect", gpu_mode));
#endif
    ...
    if (!gpu_mode || (strcmp(gpu_mode, "host") != 0 && strcmp(gpu_mode, "guest") != 0)) {
        ...
        if (!gpu_mode || !stringVectorContains(backends, gpu_mode)) {
            std::string error = StringFormat(
                ...
                "Please re-install the emulator.",
                gpu_mode);
    ...
    const bool hwGpuRequested = (emuglConfig_get_current_renderer() == SELECTED_RENDERER_HOST);
// TODO agents?
    const bool vulkanIsNotDisabled = (!fc::isOverridden(fc::Vulkan) || fc::isEnabled(fc::Vulkan));
    if (hwGpuRequested && vulkanIsNotDisabled) {
        ...
        emuglConfig_get_vulkan_hardware_gpu(&vkVendor, &vkMajor, &vkMinor, &vkPatch, nullptr,
                                            nullptr, nullptr);
        if (vkVendor) {
            ...
            if (isAMD) {
                feature_set_if_not_overridden(kFeature_VulkanAllocateDeviceMemoryOnly, true);
                if (fc::isEnabled(fc::VulkanAllocateDeviceMemoryOnly)) {
                    INFO(
                        "Enabled VulkanAllocateDeviceMemoryOnly feature for "
                        "gpu "
                        "vendor %s on Linux\n",
                        vkVendor);
                }
            ...
            hostMemoryOnWindows = isNvidia && (accelerator == ANDROID_CPU_ACCELERATOR_AEHD);
#endif
            if (hostMemoryOnLinux || hostMemoryOnWindows) {
                feature_set_if_not_overridden(kFeature_VulkanAllocateHostMemory, true);
                if (fc::isEnabled(fc::VulkanAllocateHostMemory)) {
                    INFO(
                        "Enabled VulkanAllocateHostMemory feature for "
                        "gpu "
                        "vendor %s on Linux\n",
                        vkVendor);
                }
...
void emuglConfig_setupEnv(const EmuglConfig* config) {
    if (config->use_host_vulkan) {
        ...
        if (sCurrentRenderer == SELECTED_RENDERER_SWIFTSHADER_INDIRECT ||
            sCurrentRenderer == SELECTED_RENDERER_SWIFTSHADER || strstr(config->backend, "swangle"))
#endif
        {
    ...
    if (strcmp(config->backend, "host") != 0) {
        ...
        // backend directory.
        std::string dir = sBackendList->getLibDirPath(use_swangle ? "angle" : config->backend);
        if (dir.size()) {
            D("Adding to the library search path: %s\n", dir.c_str());
            android::base::SharedLibrary::addLibrarySearchPath(dir.c_str());
        }
    }
...
#elif defined(__linux__)
    if (strstr(config->backend, "angle")) {
        android::base::setEnvironmentVariable("ANGLE_DEFAULT_PLATFORM", "vulkan");
    }
#endif
    if (use_swangle) {
        android::base::setEnvironmentVariable("ANGLE_DEFAULT_PLATFORM", "swiftshader");
    }
    ...
    std::string lib;
    if (sBackendList->getBackendLibPath(config->backend, EmuglBackendList::LIBRARY_EGL, &lib)) {
        android::base::setEnvironmentVariable("ANDROID_EGL_LIB", lib);
    }
    if (sBackendList->getBackendLibPath(config->backend, EmuglBackendList::LIBRARY_GLESv1, &lib)) {
        android::base::setEnvironmentVariable("ANDROID_GLESv1_LIB", lib);
    } else if (strcmp(config->backend, "mesa")) {
        ERR(
            "OpenGL backend '%s' without OpenGL ES 1.x library detected. "
            "Using GLESv2 only.",
            config->backend);
    ...
    if (sBackendList->getBackendLibPath(config->backend, EmuglBackendList::LIBRARY_GLESv2, &lib)) {
        android::base::setEnvironmentVariable("ANDROID_GLESv2_LIB", lib);
    }

    if (!strcmp(config->backend, "mesa")) {
        WARN("The Mesa software renderer is deprecated. ");
        WARN("Use Swiftshader (-gpu swiftshader) for software rendering.");
