...
bool emuglConfig_init(EmuglConfig* config,
    ...
    bool hasUiPreference = false;  // TODO (enum WinsysPreferredGlesBackend)uiPreferredBackend !=
    ...
    if (!strcmp(gpu_mode, "auto") || host_set_in_hwconfig) {
        ...
        // TODO if (System::get()->isRemoteSession(&sessionType)) {
        if (false) {
...
void emuglConfig_setupEnv(const EmuglConfig* config) {
    ...
    if (strcmp(config->backend, "host") != 0) {
        ...
        if (dir.size()) {
            ...
            // TODO system->addLibrarySearchDir(dir);
        }
    }
