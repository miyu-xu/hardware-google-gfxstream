...
std::vector<std::string> EmuglBackendScanner::scanDir(const char* execDir,
    ...
    if (!execDir || !android::base::pathExists(execDir)) {
        ERR("Invalid executable directory: %s", execDir);
        return names;
    }
    if (!programBitness) {
