...
public:
    EglOsEglLibrary() {
        ...
        s_eglDispatch.load(
            [&](const char* name) {
                return (void*)mLib->findSymbol(name);
            });
    }
...
public:
    EglOsEglContext(EGLDisplay display,
                    EGLContext context) :
        mDisplay(display),
        mNativeCtx(context) { }
...
public:
    ...
    void* eglGetProcAddress(const char* func) {
        return s_eglDispatch.eglGetProcAddress(func);
    }
...
EglOsEglDisplay::EglOsEglDisplay(bool nullEgl) {
    ...
    if (mDisplay == EGL_NO_DISPLAY)
        mDisplay = s_eglDispatch.eglGetDisplay(EGL_DEFAULT_DISPLAY);
    ...
    int numConfigs;
    if (s_eglDispatch.eglChooseConfig(
            mDisplay, gles3ConfigAttribs, &config, 1, &numConfigs) &&
        numConfigs != 0) {
        EGLSurface surface = s_eglDispatch.eglCreatePbufferSurface(mDisplay,
                config, pbufAttribs);
        if (surface != EGL_NO_SURFACE) {
            EGLContext ctx = s_eglDispatch.eglCreateContext(mDisplay,
                    config, EGL_NO_CONTEXT, gles31Attribs);

            if (ctx != EGL_NO_CONTEXT) {
                mGlesVersion = GlesVersion::ES31;
            } else {
                ctx = s_eglDispatch.eglCreateContext(mDisplay, config,
...
void EglOsEglDisplay::queryConfigs(int renderableType,
    ...
    s_eglDispatch.eglChooseConfig(mDisplay, framebuffer_config_attributes, configs.get(), numConfigs,
                                &numConfigs);
    CHECK_EGL_ERR
    ...
    for (int i = 0; i < numConfigs; i++) {
        ...
        s_eglDispatch.eglGetConfigAttrib(mDisplay, cfg, EGL_RENDERABLE_TYPE,
                                       &_renderableType);
        // We do emulate GLES1
        ...
        s_eglDispatch.eglGetConfigAttrib(mDisplay, cfg, EGL_RED_SIZE,
                                       &configInfo.red_size);
        s_eglDispatch.eglGetConfigAttrib(mDisplay, cfg, EGL_GREEN_SIZE,
                                       &configInfo.green_size);
        s_eglDispatch.eglGetConfigAttrib(mDisplay, cfg, EGL_BLUE_SIZE,
                                       &configInfo.blue_size);
        s_eglDispatch.eglGetConfigAttrib(mDisplay, cfg, EGL_ALPHA_SIZE,
                                       &configInfo.alpha_size);

        s_eglDispatch.eglGetConfigAttrib(mDisplay, cfg, EGL_CONFIG_CAVEAT,
                                       (EGLint*)&configInfo.caveat);
        s_eglDispatch.eglGetConfigAttrib(mDisplay, cfg, EGL_DEPTH_SIZE,
                                       &configInfo.depth_size);
        s_eglDispatch.eglGetConfigAttrib(mDisplay, cfg, EGL_LEVEL,
        ...
        s_eglDispatch.eglGetConfigAttrib(mDisplay, cfg, EGL_SAMPLES,
                                       &configInfo.samples_per_pixel);
        s_eglDispatch.eglGetConfigAttrib(mDisplay, cfg, EGL_STENCIL_SIZE,
                                       &configInfo.stencil_size);

        s_eglDispatch.eglGetConfigAttrib(mDisplay, cfg, EGL_SURFACE_TYPE,
        ...
        s_eglDispatch.eglGetConfigAttrib(mDisplay, cfg,
...
EglOsEglDisplay::createContext(EGLint profileMask,
    ...
    std::shared_ptr<Context> res =
        std::make_shared<EglOsEglContext>(mDisplay, newNativeCtx);
    D("%s done\n", __FUNCTION__);
    return res;
}
...
bool EglOsEglDisplay::makeCurrent(Surface* read,
    ...
    bool ret = s_eglDispatch.eglMakeCurrent(
