...
void pixelFormatToConfig(EGLNativeDisplayType dpy,
    ...
    if (tmp == GLX_TRANSPARENT_INDEX) {
        ...
        EXIT_IF_FALSE(glx->glXGetFBConfigAttrib(
                (Display*)dpy, frmt, GLX_TRANSPARENT_RED_VALUE, &info.trans_red_val));
        EXIT_IF_FALSE(glx->glXGetFBConfigAttrib(
                (Display*)dpy, frmt, GLX_TRANSPARENT_GREEN_VALUE, &info.trans_green_val));
        EXIT_IF_FALSE(glx->glXGetFBConfigAttrib(
                (Display*)dpy, frmt, GLX_TRANSPARENT_BLUE_VALUE, &info.trans_blue_val));
    }
    ...
    int doubleBuffer = 0;
    EXIT_IF_FALSE(glx->glXGetFBConfigAttrib(
            (Display*)dpy, frmt, GLX_DOUBLEBUFFER, &doubleBuffer));
    if (!doubleBuffer) {
        return;
    }

    EXIT_IF_FALSE(glx->glXGetFBConfigAttrib(
            (Display*)dpy ,frmt, GLX_RED_SIZE, &info.red_size));
    EXIT_IF_FALSE(glx->glXGetFBConfigAttrib(
            (Display*)dpy ,frmt, GLX_GREEN_SIZE, &info.green_size));
    EXIT_IF_FALSE(glx->glXGetFBConfigAttrib(
            (Display*)dpy ,frmt, GLX_BLUE_SIZE, &info.blue_size));
    EXIT_IF_FALSE(glx->glXGetFBConfigAttrib(
            (Display*)dpy ,frmt, GLX_ALPHA_SIZE, &info.alpha_size));
    EXIT_IF_FALSE(glx->glXGetFBConfigAttrib(
            (Display*)dpy,frmt, GLX_DEPTH_SIZE, &info.depth_size));
    EXIT_IF_FALSE(glx->glXGetFBConfigAttrib(
           (Display*)dpy ,frmt, GLX_STENCIL_SIZE, &info.stencil_size));

    info.renderable_type = renderableType;
    int nativeRenderable = 0;
    EXIT_IF_FALSE(glx->glXGetFBConfigAttrib(
           (Display*)dpy, frmt, GLX_X_RENDERABLE, &nativeRenderable));
    info.native_renderable = !!nativeRenderable;

    EXIT_IF_FALSE(glx->glXGetFBConfigAttrib(
            (Display*)dpy, frmt, GLX_X_VISUAL_TYPE, &info.native_visual_type));

    EXIT_IF_FALSE(glx->glXGetFBConfigAttrib(
            (Display*)dpy, frmt, GLX_VISUAL_ID, &info.native_visual_id));
    ...
    EXIT_IF_FALSE(glx->glXGetFBConfigAttrib(
            (Display*)dpy, frmt, GLX_MAX_PBUFFER_HEIGHT, &info.max_pbuffer_height));
    EXIT_IF_FALSE(glx->glXGetFBConfigAttrib(
            (Display*)dpy, frmt, GLX_MAX_PBUFFER_HEIGHT, &info.max_pbuffer_size));

    EXIT_IF_FALSE(glx->glXGetFBConfigAttrib(
            (Display*)dpy, frmt, GLX_LEVEL, &info.frame_buffer_level));

    EXIT_IF_FALSE(glx->glXGetFBConfigAttrib(
            (Display*)dpy, frmt, GLX_SAMPLES, &info.samples_per_pixel));
