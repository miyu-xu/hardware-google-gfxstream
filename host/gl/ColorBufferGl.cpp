...
#define GL_UNSIGNED_INT_8_8_8_8_REV       0x8367
    ...
    auto extResourceHandleInfo = ExternalObjectManager::get()->removeResourceExternalHandleInfo(hndl);
        ...
        case STREAM_PLATFORM_HANDLE_TYPE_EGL_NATIVE_PIXMAP:
            {
                ...
                cb->m_eglImage = s_egl.eglCreateImageKHR(p_display, s_egl.eglGetCurrentContext(), EGL_NATIVE_PIXMAP_KHR, nativePixmap, nullptr);
                if (cb->m_eglImage == EGL_NO_IMAGE_KHR) {
                    ERR("ColorBufferGl::create(): EGL_NATIVE_PIXMAP handle provided as external resource info, but failed to import pixmap (nativePixmap=0x%x)", nativePixmap);
                    return nullptr;
                }

                // Assume nativePixmap is compatible with ColorBufferGl's current dimensions and internal format.
                EGLBoolean setInfoRes = s_egl.eglSetImageInfoANDROID(p_display, cb->m_eglImage, cb->m_width, cb->m_height, cb->m_internalFormat);
            ...
            break;
        default:
            ERR("ColorBufferGl::create -- external memory info was provided, but ", p_internalFormat);
            return nullptr;
        }
    } else {
