            ...
            if (tInfo->m_vkInfo) {
                ...
                if (!tInfo->m_vkInfo->m_vkDec.getVkDecoderGlobalState()) {
                    if (FrameBuffer::getFB()
                            ->getEmulationVk()
                            .getFeatures()
                            .VulkanSeparateGlobalState.enabled) {
                        if (tInfo->m_puid && tInfo->m_puid != INVALID_CONTEXT_ID) {
                            auto gsptr =
                                vk::VkDecoderGlobalState::getVkDecoderGlobalState(tInfo->m_puid);
                            auto* renderThreadInfo = vk::RenderThreadInfoVk::get();
                            tInfo->m_vkInfo->m_vkDec.setVkDecoderGlobalState(gsptr);
                        }
                    } else {
                        auto gsptr = vk::VkDecoderGlobalState::getVkDecoderGlobalStateById(0);
                        tInfo->m_vkInfo->m_vkDec.setVkDecoderGlobalState(gsptr);
                    }
                }
