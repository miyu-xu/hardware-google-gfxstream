...
static bool shouldEnableAsyncSwap(const gfxstream::host::FeatureSet& features) {
    ...
    return features.GlAsyncSwap.enabled && emugl_sync_device_exists() &&
...
static bool shouldEnableVulkanAsyncQsri(const gfxstream::host::FeatureSet& features) {
    return shouldEnableVulkan(features) &&
           (features.GlAsyncSwap.enabled ||
            (features.VirtioGpuNativeSync.enabled && features.VirtioGpuFenceContexts.enabled));
}
...
static bool shouldEnableBatchedDescriptorSetUpdate(const gfxstream::host::FeatureSet& features) {
    return shouldEnableVulkan(features) && shouldEnableQueueSubmitWithCommands(features) &&
...
static EGLint rcGetGLString(EGLenum name, void* buffer, EGLint bufferSize) {
    ...
    bool vulkanFreeMemorySyncEnabled = shouldEnableVulkan(features);
...
static void rcCreateSyncKHR(EGLenum type,
    ...
    fb->createEmulatedEglFenceSync(type, destroyWhenSignaled, outSync, outSyncThread);
