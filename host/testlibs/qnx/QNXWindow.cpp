...
bool QNXWindow::initialize(const std::string& name, size_t width,
                           size_t height) {
    ...
    rc = screen_create_window_type(&screen_window, screen_ctx,
    ...
    screen_set_window_property_iv(screen_window, SCREEN_PROPERTY_ALPHA_MODE,
                                  &alpha_mode);
    ...
    int interval = 1;
    screen_set_window_property_iv(screen_window, SCREEN_PROPERTY_SWAP_INTERVAL,
                                  &interval);
    ...
    size[0] = width;
    size[1] = height;
    screen_set_window_property_iv(screen_window, SCREEN_PROPERTY_BUFFER_SIZE,
                                  size);
    ...
    screen_set_window_property_iv(screen_window, SCREEN_PROPERTY_SENSITIVITY,
                                  &sensitivity);
    ...
    screen_set_window_property_iv(screen_window, SCREEN_PROPERTY_VISIBLE,
                                  &visible);
...
void QNXWindow::messageLoop() {
    ...
    while (!screen_get_event(get_screen_context(), screen_event, 0)) {
        int event_type;
        rc = screen_get_event_property_iv(screen_event, SCREEN_PROPERTY_TYPE,
                                          &event_type);
...
void QNXWindow::setMousePosition(int x, int y) {
    ...
    if (screen_set_event_property_iv(screen_event, SCREEN_PROPERTY_TYPE,
                                     &param))
        return;

    if (screen_set_event_property_pv(screen_event, SCREEN_PROPERTY_WINDOW,
                                     (void**)&mWindow))
        return;
    int coords[] = {x, y};
    screen_set_event_property_iv(screen_event, SCREEN_PROPERTY_SOURCE_POSITION,
                                 coords);
...
void QNXWindow::setVisible(bool isVisible) {
    ...
    if (!screen_set_window_property_iv(mWindow, SCREEN_PROPERTY_VISIBLE,
                                       &visible))
        mVisible = isVisible;
}
...
void QNXWindow::signalTestEvent() {
    ...
    screen_set_event_property_iv(screen_event, SCREEN_PROPERTY_BUTTONS,
                                 &button);
    screen_send_event(get_screen_context(), screen_event, mPid);
}
...
void QNXWindow::processMouseEvent(const screen_event_t& screen_event) {
    ...
    int wheel_ticks = 0;
    screen_get_event_property_iv(screen_event, SCREEN_PROPERTY_MOUSE_WHEEL,
                                 &wheel_ticks);
    if (wheel_ticks)
    ...
    switch (event.Type) {
        ...
        case Event::EVENT_MOUSE_MOVED:
            ...
            screen_get_event_property_iv(
                screen_event, SCREEN_PROPERTY_SOURCE_POSITION, position);
...
void QNXWindow::processKeyEvent(const screen_event_t& screen_event) {
    ...
    screen_get_event_property_iv(screen_event, SCREEN_PROPERTY_MODIFIERS,
                                 &modifiers);
    ...
    event.Type = flags & KEY_DOWN || flags & KEY_REPEAT
...
void QNXWindow::processPropertyChangedEvent(const screen_event_t& event) {
    ...
    if (property == SCREEN_PROPERTY_SIZE) {
        ...
        if (screen_get_event_property_pv(event, SCREEN_PROPERTY_WINDOW,
                                         (void**)&screenWindow))
            return;
        if (screenWindow != mWindow) return;

        int size[2] = {0, 0};
        if (screen_get_window_property_iv(screenWindow, SCREEN_PROPERTY_SIZE,
                                          size))
            return;
        Event event;
        ...
        if (screenWindow != mWindow) return;
        int value;
        if (screen_get_window_property_iv(screenWindow, property, &value))
            return;
        Event event;
        event.Type =
            value ? Event::EVENT_GAINED_FOCUS : Event::EVENT_LOST_FOCUS;
    }
}
...
void QNXWindow::processInputControlEvent(const screen_event_t& screen_event) {
    ...
    if (screen_get_event_property_iv(screen_event,
                                     SCREEN_INPUT_CONTROL_POINTER_STOP, &val))
        return;
    if (!val) return;
    screen_window_t screenWindow = 0;
    if (screen_get_event_property_pv(screen_event, SCREEN_PROPERTY_WINDOW,
                                     (void**)&screenWindow))
        return;
    if (screenWindow != mWindow) return;
    Event event;
