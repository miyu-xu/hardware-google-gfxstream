...
std::optional<VirtioGpuResource> VirtioGpuResource::Create(
    ...
    if (resourceType == VirtioGpuResourceType::PIPE) {
        ...
        gfxstream::FrameBuffer::getFB()->createBufferWithHandle(args->width * args->height, args->handle);
        ...
        const bool linear =
            ...
            false;
#endif
        gfxstream::FrameBuffer::getFB()->createColorBufferWithHandle(args->width, args->height, glformat, fwkformat, args->handle, linear);
        gfxstream::FrameBuffer::getFB()->setGuestManagedColorBufferLifetime(true /* guest manages lifetime */);
...
VirtioGpuResource::Create(const gfxstream::host::FeatureSet& features,
                          uint32_t contextId,
                          uint32_t resourceId,
    ...
    if (createBlobArgs->blob_id == 0) {
        if (features.ExternalBlob.enabled) {
            resource.ringBlob = RingBlob::CreateWithShmem(resourceId, createBlobArgs->size);
        } else {
            resource.ringBlob = RingBlob::CreateWithHostMemory(resourceId, createBlobArgs->size, mPageSize);
        }
        if (!resource.ringBlob) {
            stream_renderer_error("Failed to create blob: failed to create ring blob.");
            return std::nullopt;
        }
        resource.hva = entry.ringBlob->map();
        resource.hvaSize = createBlobArgs->size;
        resource.externalAddr = true;
        resource.caching = STREAM_RENDERER_MAP_CACHE_CACHED;
        ...
        if (createBlobArgs->blob_mem == STREAM_BLOB_MEM_GUEST &&
            ...
            if (!descriptorInfoOpt) {
                descriptorInfoOpt = ExternalObjectManager::get()->removeBlobDescriptorInfo(
                    ctx_id, createBlobArgs->blob_id);
            }

            if (descriptorInfoOpt) {
                resource.descriptorInfo = std::make_shared<BlobDescriptorInfo>(std::move(*descriptorInfoOpt));
            } else {
                stream_renderer_error("Failed to create blob: no external blob descriptor.");
                return std::nullopt;
            }

            resource.caching = resource.descriptorInfo->caching;
        }
    } else {
        auto entryOpt = ExternalObjectManager::get()->removeMapping(ctx_id, createBlobArgs->blob_id);
        if (entryOpt) {
            resource.hva = entryOpt->addr;
            resource.hvaSize = createBlobArgs->size;
            resource.caching = entryOpt->caching;
        } else {
...
int VirtioGpuResource::Destroy() {
    if (mResourceType == VirtioGpuResourceType::BUFFER) {
        gfxstream::FrameBuffer::getFB()->closeBuffer(mId);
    } else if (mResourceType == VirtioGpuResourceType::COLOR_BUFFER) {
        gfxstream::FrameBuffer::getFB()->closeColorBuffer(mId);
    }
    return 0;
}
...
int VirtioGpuResource::GetInfo(struct stream_renderer_resource_info* outInfo) const {
    if (!mCreateArgs) {
        stream_renderer_error("Failed to get info: resource %d missing args.", resourceId);
        return ENOENT;
    }
    ...
    info->stride = align_up(mCreateArgs->width * bpp, 16U);
    info->virgl_format = mCreateArgs->format;
    info->handle = mCreateArgs->handle;
    info->height = mCreateArgs->height;
    info->width = mCreateArgs->width;
    info->depth = mCreateArgs->depth;
    info->flags = mCreateArgs->flags;
    info->tex_id = 0;
    return 0;
}
...
int VirtioGpuResource::TransferRead(const GoldfishPipeServiceOps* ops,
    ...
    if (iovec_cnt) {
        ret = entry.TransferToIov(offset, box, std::vector<struct iovec>(iov, iov + iovec_cnt));
    } else {
        ret = entry.TransferToIov(offset, box);
    }
    if (ret != 0) {
...
VirtioGpuResource::TransferWriteResult
VirtioGpuResource::TransferWrite(const GoldfishPipeServiceOps* ops,
                                 uint64_t offset, stream_renderer_box* box,
                                 std::optional<std::vector<struct iovec>> iovs) {
    // First, copy from the desired iov to this resource's linear buffer:
    int ret = 0;
    if (iovec_cnt) {
        ret = entry.TransferFromIov(offset, box, std::vector<struct iovec>(iov, iov + iovec_cnt));
    } else {
        ret = entry.TransferFromIov(offset, box);
    }
    if (ret != 0) {
...
int VirtioGpuResource::ReadFromPipeToLinear(const GoldfishPipeServiceOps* ops, uint64_t offset, stream_renderer_box* box) {
...
VirtioGpuResource::TransferWriteResult
VirtioGpuResource::WriteToPipeFromLinear(const GoldfishPipeServiceOps* ops,
                                         uint64_t offset, stream_renderer_box* box) {
    if (mResourceType != VirtioGpuResourceType::PIPE) {
        stream_renderer_error("Failed to transfer: resource %d is not PIPE.", mId);
        return -EINVAL;
...
int VirtioGpuResource::ReadFromBufferToLinear(uint64_t offset, stream_renderer_box* box) {
    ...
    gfxstream::FrameBuffer::getFB()->readBuffer(
        mCreateArgs->handle, 0, mCreateArgs->width * mCreateArgs->height, mLinear.data());
    return 0;
}
...
int VirtioGpuResource::ReadFromColorBufferToLinear(uint64_t offset, stream_renderer_box* box) {
    ...
    if (virgl_format_is_yuv(mCreateArgs->format)) {
        gfxstream::FrameBuffer::getFB()->readColorBufferYUV(
            ...
            mLinear.data(), mLinear.data()Size);
    } else {
        gfxstream::FrameBuffer::getFB()->readColorBuffer(
            mCreateArgs->handle, 0, 0, mCreateArgs->width, mCreateArgs->height,
            glformat, gltype, mLinear.data());
    }
}
...
int VirtioGpuResource::WriteToColorBufferFromLinear(uint64_t offset, stream_renderer_box* box) {
    ...
    gfxstream::FrameBuffer::getFB()->updateColorBuffer(
        mCreateArgs->handle, 0, 0, mCreateArgs->width, mCreateArgs->height, glformat,
        gltype, mLinear.data());
    return 0;
}
...
int VirtioGpuResource::TransferWithIov(uint64_t offset, const stream_renderer_box* box,
    ...
    size_t linearBase =
        virgl_format_to_linear_base(mCreateArgs->format, mCreateArgs->width,
                                    mCreateArgs->height, box->x, box->y, box->w, box->h);
    ...
    size_t length =
        virgl_format_to_total_xfer_len(mCreateArgs->format, mCreateArgs->width,
                                       mCreateArgs->height, box->x, box->y, box->w, box->h);
    ...
    size_t iovOffset = 0;
    size_t written = 0;
    char* linear = static_cast<char*>(mLinear);
...
/*static*/
std::optional<VirtioGpuResourceSnapshot> VirtioGpuResource::Snapshot() {
...
/*static*/ std::optional<VirtioGpuResource> VirtioGpuResource::Restore(
    ...
    if (resourceSnapshot.has_ring_blob()) {
        ...
        resource.mHva = mRingBlob->map();
        resource.mHvaSize = mRingBlob->size();
    }
