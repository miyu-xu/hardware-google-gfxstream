...
std::optional<VirtioGpuResource> VirtioGpuResource::Create(
    ...
    if (!import_handle || !import_data) {
        ...
        stream_renderer_error("Failed to import resource: stream_renderer_3d_info not provided in import data.");
        return std::nullopt;
    }
    ...
    if (!virglFormat) {
        stream_renderer_error("No virgl format available for drm_fourcc: %d", import_data->info_3d.drm_fourcc);
        return std::nullopt;
    }
    ...
    if (resourceType != VirtioGpuResourceType::COLOR_BUFFER) {
        stream_renderer_error("Failed to create resource with import_handle: arguments resulted in unhandled type. Only ColorBuffer resources are supported for import.");
        return std::nullopt;
    }

    ExternalObjectManager::get()->addResourceExternalHandleInfo(res_handle,
        ExternalHandleInfo {
    ...
    const auto fwkformat = (gfxstream::FrameworkFormat)virgl_format_to_fwk_format(internal_create_args.format);
    const bool linear =
#ifdef GFXSTREAM_ENABLE_GUEST_VIRTIO_RESOURCE_TILING_CONTROL
        ...
        false;
#endif
    FrameBuffer::getFB()->createColorBufferWithResHandle(internal_create_args.width, internal_create_args.height, glformat,
                                                        fwkformat, internal_create_args.handle, linear);
    FrameBuffer::getFB()->setGuestManagedColorBufferLifetime(true /* guest manages lifetime */);
