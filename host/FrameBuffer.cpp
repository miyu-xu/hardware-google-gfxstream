...
void MaybeIncreaseFileDescriptorSoftLimit() {
    ...
    if (softLimit < kDesiredNofileSoftLimit) {
        ...
        const rlim_t possibleSoftLimit = std::min(kDesiredNofileSoftLimit, nofileLimits.rlim_max);

        struct rlimit requestedNofileLimits = {
            .rlim_cur = possibleSoftLimit,
            .rlim_max = hardLimit,
        };
        ...
        if (ret) {
            ERR("Warning: failed to raise nofile soft limit to %d: %s (%d)",
                static_cast<int>(possibleSoftLimit), strerror(errno), errno);
            return;
        }

        GL_LOG("Raised nofile soft limit to %d.", static_cast<int>(possibleSoftLimit));
    }
#endif
}
