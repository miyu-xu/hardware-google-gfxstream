...
bool FrameBuffer::initialize(int width, int height, gfxstream::host::FeatureSet features,
    ...
    if (!fb->m_features.GuestUsesAngle.enabled) {
        fb->m_emulationGl =
            EmulationGl::create(width, height, fb->m_features, useSubWindow, egl2egl);
        if (!fb->m_emulationGl) {
    ...
    fb->m_useVulkanComposition =
        fb->m_features.GuestUsesAngle.enabled || fb->m_features.VulkanNativeSwapchain.enabled;
...
FrameBuffer::FrameBuffer(int p_width, int p_height, gfxstream::host::FeatureSet features,
                         bool useSubWindow)
    : m_features(features),
...
void FrameBuffer::createYUVTextures(uint32_t type, uint32_t count, int width, int height,
    ...
    for (uint32_t i = 0; i < count; ++i) {
        if (format == FRAMEWORK_FORMAT_NV12) {
            YUVConverter::createYUVGLTex(GL_TEXTURE0, width, height, format,
                                         m_features.Yuv420888ToNv21.enabled, YUVPlane::Y,
                                         &output[2 * i]);
            YUVConverter::createYUVGLTex(GL_TEXTURE1, width / 2, height / 2, format,
                                         m_features.Yuv420888ToNv21.enabled, YUVPlane::UV,
                                         &output[2 * i + 1]);
            ...
            YUVConverter::createYUVGLTex(GL_TEXTURE0, width, height, format,
                                         ...
                                         &output[3 * i]);
            YUVConverter::createYUVGLTex(GL_TEXTURE1, width / 2, height / 2, format,
                                         m_features.Yuv420888ToNv21.enabled, YUVPlane::U,
                                         &output[3 * i + 1]);
            YUVConverter::createYUVGLTex(GL_TEXTURE2, width / 2, height / 2, format,
                                         m_features.Yuv420888ToNv21.enabled, YUVPlane::V,
                                         &output[3 * i + 2]);
        }
    }
}
