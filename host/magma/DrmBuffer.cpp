...
void* DrmBuffer::map() {
    ...
    drm_prime_handle handle_params{.handle = mGemHandle};
    result = mDevice.ioctl(DRM_IOCTL_PRIME_HANDLE_TO_FD, &handle_params);
    if (result) {
    ...
    BlobManager::get()->addDescriptorInfo(mContextId, mId,
    ...
    INFO("Mapped DrmBuffer size %" PRIu64 " gem %" PRIu32 " to addr %p fd %" PRIu32 " mid %" PRIu64,
         mSize, mGemHandle, mHva, handle_params.fd, mId);
